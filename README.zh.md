...
SleepyCode 不支持 macOS 自带的“终端”（Terminal.app）。如果界面出现错位、闪烁或其他渲染异常，请改用 [iTerm2](https://iterm2.com/) 或 VS Code 集成终端：
...
如果通过 SSH 直接运行 TUI 时卡顿，可以让 TUI 在本地渲染，远端只运行 SleepyCode 服务。先在远端项目目录中启动服务：
...
`sleepy models <provider>` 会逐个模型打印 SleepyCode 解析出的窗口以及实际压缩的 token 数。输入框底部
...
Compose 是 SleepyCode 的 specs-driven 结构化开发流程，编排从 spec 到交付的完整开发生命周期。
...
**自定义 Workflow：** 在 `.sleepycode/workflows/` 或 `.claude/workflows/` 下放置 `.js` 文件即可定义自己的 Workflow，也可用同名文件覆盖内置 Workflow（如 `.sleepycode/workflows/compose.js`）。

### 内置技能（Builtin Skills）

技能（Skill）是可复用的指令集，教会 Agent 如何处理特定任务（如生成 PDF、写学术论文、搜索 arXiv）。面对新任务时，SleepyCode 会按准确名称、本地化别名和 BM25 相关性搜索可用的非 Compose Skill；高置信度结果会自动加载，不确定的候选则交由 Agent 判断。在 TUI 中输入 `/` 可以浏览自动补全列表，也可以通过 `/<skill-name>` 直接调用 Skill——一条消息里提到两个及以上技能时会自动加载它们并注入多技能编排计划。

SleepyCode 打包了以下内置技能：
...
| `loop` | 按固定周期调度循环提示 |
| `sleepycode-docs` | SleepyCode 功能、命令、Provider 和配置的自文档参考 |
...
**覆盖内置技能：** 在项目（`.sleepycode/skills/<name>/SKILL.md`）或个人技能目录（`~/.claude/skills/`、`~/.opencode/skills/` 等）中创建同名技能即可。扫描顺序中后发现的用户技能会覆盖同名的内置技能。
...
| 变量 | 效果 |
|------|------|
| `SLEEPYCODE_DISABLE_BUILTIN_SKILLS=true` | 禁用所有内置技能 |
| `SLEEPYCODE_DISABLE_OFFICIAL_SKILLS=true` | 仅禁用办公/媒体类技能：`docx-official`、`pdf-official`、`pptx-official`、`xlsx-official`、`html-to-video-pipeline` |
| `SLEEPYCODE_DISABLE_SLASH_SKILLS=true` | 从 TUI 的 `/` 自动补全中隐藏 Skill，但不禁用它们 |

前两个选项会将对应技能从 Agent 可用技能列表中完全移除——不会出现在上下文中，也无法被调用。`SLEEPYCODE_DISABLE_SLASH_SKILLS` 仅影响 TUI 自动补全，Skill 对 Agent 仍然可用。
...
SleepyCode 使用 JSON/JSONC 配置文件，并提供 JSON Schema 以获得编辑器自动补全和校验。

### 文件位置

| 文件 | 项目级 | 全局 |
|------|--------|------|
| 主配置 | `.sleepycode/sleepycode.jsonc`（也支持 `.json`） | `~/.config/sleepycode/sleepycode.jsonc`（也支持 `.json`） |
| TUI 配置 | `.sleepycode/tui.json` | `~/.config/sleepycode/tui.json` |
| 认证凭据 | — | `~/.local/share/sleepycode/auth.json` |

> Windows 下 XDG 路径位于 `%LOCALAPPDATA%\sleepycode\`。可通过 `SLEEPY_HOME` 环境变量覆盖所有路径。

### JSON Schema

SleepyCode 在首次加载配置时会自动注入 `$schema` 字段，使编辑器开箱即获得补全和校验：
...
除配置文件外，SleepyCode 在 XDG 路径（或 `$SLEEPY_HOME`）下存储运行时数据：

| 目录 | 默认路径（Linux） | 内容 |
|------|------------------|------|
| data | `~/.local/share/sleepycode/` | SQLite 数据库、认证凭据（`auth.json`）、记忆、日志 |
| state | `~/.local/state/sleepycode/` | TUI 偏好设置（`kv.json`）、最近使用模型（`model.json`） |
| cache | `~/.cache/sleepycode/` | 语言服务器、缓存的模型目录、技能 |

如需删除已存储的凭据，删除 data 目录下的 `auth.json` 即可。macOS 下 XDG data 默认为 `~/Library/Application Support/sleepycode/`。
...
- 原样保留用户提供的 Base URL 和模型 ID。SleepyCode 不要求 Provider 已存在于内置目录中；除非端点本身要求，否则不要自行增删 `/v1`。
...
全局配置请写入 `~/.config/sleepycode/sleepycode.jsonc`（或同目录的 `sleepycode.json`），仅项目生效的配置请写入 `.sleepycode/sleepycode.jsonc`（或 `.json`），并与已有内容合并。`apiKey` 会以明文保存在配置中，请确保文件仅当前用户可读，且不要提交到版本库。可运行 `sleepy models` 或使用 TUI 模型选择器验证配置结果。
...
# 无头模式
sleepy run --dangerously-skip-permissions "你的提示词"
