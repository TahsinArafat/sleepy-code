---
name: drive-sleepy
description: Use when developing or testing the SleepyCode repository and you need to programmatically drive another SleepyCode (sleepy) process. Supports headless `sleepy run` with JSON events and interactive TUI via tmux for behavior, integration, and visual regression testing. Covers an installed `sleepy` binary or a dev build launched from source with `bun dev`; do not use it for ordinary SleepyCode task execution.
---
...
Two **interfaces** for driving a separate sleepy process:

| Interface | Command | Use when |
|------|---------|----------|
| **Headless** | `sleepy run --format json` | Scripted tasks, CI, event validation |
| **TUI** | `sleepy` in tmux | Interactive flow, permission dialogs, keybindings, visual regression |

**Core principle:** Every run produces parseable evidence. No eyeballing.

Always drive an *isolated* instance: give each run a fresh `SLEEPY_HOME` and a throwaway workspace so it never touches your own config, memory, or session DB.
...
|----------|---------|----------|
| **Installed binary** | `sleepy …` | Testing a released/installed build |
| **Dev (from source)** | `bun dev …` | Debugging sleepycode itself — runs `src/index.ts` directly, no build step, picks up local code changes |

Everything below uses `sleepy` for brevity. To drive a **dev build** instead,
substitute `bun dev` for `sleepy` and run from the repo root — every flag,
JSON event, and tmux technique is identical. See the Dev Mode section.

## Prerequisites

```bash
# sleepy binary must be on PATH (installed-binary launcher)
which sleepy || echo "sleepy not found on PATH"

# OR: dev launcher — run from the sleepycode repo root, needs bun
...
## Dev Mode (debugging sleepycode itself)

When the goal is to debug **sleepycode's own code**, launch from source with
`bun dev` instead of the installed `sleepy` binary. It runs
...
- It is the dev equivalent of the `sleepy` command: same CLI, same subcommands
...
- Args pass straight through, so **both interfaces work under dev**:
  ...
  - TUI:      `bun dev <workspace>` (positional workspace arg, as with `sleepy`)
- If `SLEEPY_HOME` is not set, dev defaults it to a repo-local `.dev-home`
  dir. For an isolated driven run, set `SLEEPY_HOME=$(mktemp -d)` explicitly
  just like with the binary.

**Substitution rule:** anywhere Part 1 / Part 2 / Part 3 below say `sleepy`,
replace it with `bun dev` (invoked from the repo root) to drive a dev build.

```bash
# Headless, dev build (from repo root)
REPO=/path/to/sleepycode/checkout   # your local sleepycode repo root
SLEEPY_HOME=$(mktemp -d) bun --cwd "$REPO" dev run \
  --format json --dangerously-skip-permissions --dir "$WORKSPACE" \
  < "$PROMPT" > /tmp/sleepy-dev.jsonl 2>&1

# TUI, dev build in tmux (from repo root)
tmux new-session -d -s "$SESSION" -x 120 -y 30 \
  "cd $REPO && SLEEPY_HOME=$(mktemp -d) SLEEPYCODE_PURE=true bun dev $WORKSPACE; sleep 999"
```

---

## Part 1: Headless Mode (`sleepy run`)

### Launch

```bash
PROMPT=$(mktemp -t sleepy-drive.XXXXXX)
cat >"$PROMPT" <<'EOF'
Your task here.
EOF

SLEEPY_HOME=$(mktemp -d) sleepy run \
  --format json \
  --dangerously-skip-permissions \
  --dir "$WORKSPACE" \
  < "$PROMPT" > /tmp/sleepy-out.jsonl 2>&1
...
# No errors
grep -q '"type":"error"' /tmp/sleepy-out.jsonl && echo "FAIL: errors found"

# A specific tool was used (match part.tool, the bare string)
grep -q '"tool":"write"' /tmp/sleepy-out.jsonl || echo "FAIL: write tool not called"

# Text output contains expected string (text is at .part.text)
grep '"type":"text"' /tmp/sleepy-out.jsonl | jq -r '.part.text' | grep -q "expected"

# Robust tool check via jq (works regardless of key ordering)
jq -e 'select(.type=="tool_use") | .part.tool=="write"' /tmp/sleepy-out.jsonl >/dev/null \
...
```bash
timeout 120 sleepy run --format json --dangerously-skip-permissions < "$PROMPT"
...
| Variable | Purpose |
|---|---|
| `SLEEPY_HOME` | **Required.** Fresh `mktemp -d` per run. Sandboxes DB, config, cache. |
| `SLEEPYCODE_PURE=true` | Disable external plugins. |
| `SLEEPYCODE_DISABLE_GIT=true` | Skip git ops if workspace isn't a real repo. |
...
MHOME=$(mktemp -d)
WORKSPACE=$(mktemp -d)
SESSION="sleepy-drive-$$"

# Launch sleepy TUI in tmux (workspace is a positional arg, NOT --dir)
tmux new-session -d -s "$SESSION" -x 120 -y 30 \
  "SLEEPY_HOME=$MHOME SLEEPYCODE_PURE=true sleepy $WORKSPACE; sleep 999"
...
test_smoke() {
  ...
  echo "Say hello" > "$P"
  SLEEPY_HOME=$MHOME sleepy run --format json --dangerously-skip-permissions \
    < "$P" > /tmp/s1.jsonl 2>&1
  local E=$?
  rm -rf "$MHOME" "$P"
...
test_tool_use() {
  ...
  SLEEPY_HOME=$MHOME sleepy run --format json --dangerously-skip-permissions --dir "$WS" \
    < "$P" > /tmp/s2.jsonl 2>&1
  local E=$?
  local OK=true
  [ $E -ne 0 ] && OK=false
...
test_tui_interactive() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="sleepy-s3-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "SLEEPY_HOME=$MHOME SLEEPYCODE_PURE=true sleepy $WS; sleep 999"
...
```bash
test_tui_permission() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="sleepy-s4-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "SLEEPY_HOME=$MHOME SLEEPYCODE_PURE=true sleepy $WS; sleep 999"
...
test_tui_keybindings() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="sleepy-s5-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "SLEEPY_HOME=$MHOME SLEEPYCODE_PURE=true sleepy $WS; sleep 999"
...
| Task | Command |
|------|---------|
| Headless run | `SLEEPY_HOME=$(mktemp -d) sleepy run --format json --dangerously-skip-permissions "prompt"` |
| TUI launch | `tmux new-session -d -s test -x 120 -y 30 "SLEEPY_HOME=$(mktemp -d) sleepy $WORKSPACE; sleep 999"` |
