...
// ── Auth ──────────────────────────────────────────────────────────────────────
interface GatewayConfig { access_token?: string; refresh_token?: string; expires_at?: number; dashboard_url?: string; }
interface SleepyModel { id: string; name: string; contextWindow: number; }
...
class Auth {
  ...
  async fetchModels() {
    ...
    try {
      ...
      if (r.ok) { const j = await r.json(); this.models = (j.data ?? []).map((m: any) => ({ id: m.modelId ?? m.id, name: m.name ?? m.id, contextWindow: m.contextWindow ?? 128000 })); }
    } catch {}
  }
}
...
class SleepyWebview implements vscode.WebviewViewProvider {
  private _v?: vscode.WebviewView; private _abort = new AbortController();
  ...
  async handle(m: any) {
    switch (m.t) {
      case "init": this.push({ t: "auth", authed: this._auth.authed, models: this._auth.models }); break;
      case "chat": await this.chat(m.text, m.model); break;
      case "abort": this._abort.abort(); break;
    }
  }

  async chat(text: string, modelId: string) {
    if (!this._auth.token) { this.push({ t: "err", msg: "Not logged in. Run: sleepy login" }); return; }
    ...
    try {
      ...
      if (!r.ok) { this.push({ t: "err", msg: `API ${r.status}: ${r.statusText}` }); return; }
      ...
      while (reader) {
        ...
        for (const l of lines) {
          ...
          const d = t.slice(6);
          if (d === "[DONE]") continue;
          try { const p = JSON.parse(d); const dt = p.choices?.[0]?.delta?.content; if (dt) this.push({ t: "stream", text: dt }); } catch {}
        }
      }
      this.push({ t: "streamEnd" });
    } catch (e: any) { if (e.name !== "AbortError") this.push({ t: "err", msg: e.message || "Request failed" }); }
  }
}

// ── HTML ──────────────────────────────────────────────────────────────────────
function H(auth: Auth): string {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const w = setInterval(async () => {
    try {
      ...
      if (mt > lastMtime) { lastMtime = mt; auth.read(); await auth.fetchModels(); }
    } catch {}
  }, 3000);
