...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-sleepycode-directory", "directory"],
    ["x-sleepycode-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-sleepycode-directory")
  next.headers.delete("x-sleepycode-workspace")
  return next
}
...
export function createOpencodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-sleepycode-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-sleepycode-workspace": config.experimental_workspaceID,
    }
  }
