...
export const Path = iife(() => {
  if (Flag.SLEEPYCODE_DB) {
    if (Flag.SLEEPYCODE_DB === ":memory:" || path.isAbsolute(Flag.SLEEPYCODE_DB)) return Flag.SLEEPYCODE_DB
    return path.join(Global.Path.data, Flag.SLEEPYCODE_DB)
  }
...
export const Client = lazy(() => {
  ...
  if (entries.length > 0) {
    ...
    if (Flag.SLEEPYCODE_SKIP_MIGRATIONS) {
