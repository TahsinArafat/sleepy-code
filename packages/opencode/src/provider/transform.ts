...
export function usesLargeModelDefaults(model: { id: string; providerID: string; api: { id: string } }) {
  if (["sleepy", "xiaomi"].includes(model.providerID.toLowerCase())) return true
  return [model.id, model.api.id].some((id) =>
    ["claude", "gpt", "sleepy"].some((name) => id.toLowerCase().includes(name)),
  )
}
...
function forceAnthropicReasoningContent(msgs: ModelMessage[], model: Provider.Model): ModelMessage[] {
  if (!Flag.SLEEPYCODE_FORCE_ANTHROPIC_REASONING_CONTENT) return msgs
...
// explicit Flag.SLEEPYCODE_MAX_PROMPT_IMAGE_SIZE byte override is set.
//
...
function limitImages(msgs: ModelMessage[], model: Provider.Model): ModelMessage[] {
  ...
  const maxSize = Flag.SLEEPYCODE_MAX_PROMPT_IMAGE_SIZE ?? providerImageCap(model)
...
// sleepy-v2.5-pro / MiniMax-M3, which stringify the whole envelope without it —
