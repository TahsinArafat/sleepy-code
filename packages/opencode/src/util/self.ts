 ...
 *
 * `sleepy` is frequently not a command. It may have been launched through `npx`, from
 * a `node_modules/.bin` shim, from a source checkout with `bun run dev`, or from
 * inside a host application's bundle. Printing "run `sleepy ...`" in that situation
 * is advice the caller cannot follow.
 *
 * The reliable signal is the running process itself. Every SHIPPED path ends up
 * executing the compiled single-file binary — `bin/sleepy` is only a Node shim that
...
export function argv(...args: string[]): string[] {
  // Honours the same override `bin/sleepy` reads, so a host that pins a specific
  // binary keeps control of which one a re-invocation reaches.
  const pinned = process.env["SLEEPYCODE_BIN_PATH"]
