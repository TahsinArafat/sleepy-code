 ...
 *
 * `SLEEPYCODE_AUTH_CONTENT` is the whole `auth.json` (model provider keys, OAuth refresh tokens);
 * `SLEEPYCODE_CONFIG_CONTENT` can embed MCP `environment` values and request `headers`.
 *
 ...
 * so plain inheritance means `echo $SLEEPYCODE_AUTH_CONTENT` exfiltrates the provider key.
 *
 ...
 */
const CREDENTIAL_ENV = new Set(["SLEEPYCODE_AUTH_CONTENT", "SLEEPYCODE_CONFIG_CONTENT"])
