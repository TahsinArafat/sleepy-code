...
export type PatchDeps = {
  ...
  files: (dir: string, name: "sleepycode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".sleepycode")
}

function patchName(kind: Kind): "sleepycode" | "tui" {
  if (kind === "server") return "sleepycode"
  return "tui"
}
