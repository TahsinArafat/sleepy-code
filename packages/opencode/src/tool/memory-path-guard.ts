...
/** Agents that may write memory or the project's `.sleepycode/` directory. The
 ...
 * memory-only; dream/distill may also write under `<worktree>/.sleepycode/`.
...
export function assertAgentWriteSandbox(input: {
  ...
  // `<worktree>/.sleepycode/../src/x.ts` would string-prefix-match `.sleepycode`
  ...
  const dotDir = path.resolve(input.worktree, ".sleepycode")
