...
import { Slug } from "@sleepy-ai/shared/util/slug"
...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    SLEEPYCODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    SLEEPYCODE_WORKSPACE_ID: config.id,
    SLEEPYCODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.SLEEPYCODE_EXPERIMENTAL_WORKSPACES) return
