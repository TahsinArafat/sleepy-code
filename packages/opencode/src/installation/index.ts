...
const PACKAGE_NAME = "@sleepy-ai/cli"
...
export const USER_AGENT = `sleepycode/${InstallationChannel}/${InstallationVersion}/${Flag.SLEEPYCODE_CLIENT}`
...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | ChildProcessSpawner.ChildProcessSpawner> =
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // TODO(sleepycode): uncomment when sleepycode is published to homebrew
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          ...
          const response = yield* httpOk.execute(HttpClientRequest.get(process.env.SLEEPYCODE_INSTALL_SCRIPT_URL ?? "https://mimo.xiaomi.com/install"))
      ...
      const upgradeCurlWindows = Effect.fnUntraced(function* (target: string) {
        ...
        const installScriptUrl = process.env.SLEEPYCODE_INSTALL_SCRIPT_URL ?? "https://mimo.xiaomi.com/install.ps1"
        const downloadResult = yield* run(
          ...
          { env: { SLEEPYCODE_INSTALL_DIR: stageDir, VERSION: target, INSTALL_SCRIPT_URL: installScriptUrl } },
        )
      ...
      const methodImpl = Effect.fn("Installation.method")(function* () {
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          // TODO(sleepycode): uncomment when sleepycode is published to these channels
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "curl") {
          ...
          // SLEEPY_FDS_BASE to mirror the install script.
          const base = (process.env.SLEEPY_FDS_BASE || "https://sleepycode.cnbj1.mi-fds.com/sleepycode/sleepycode").replace(
            /\/+$/,
            "",
          )
        ...
        // TODO(sleepycode): uncomment when sleepycode is published to chocolatey
        ...
        // TODO(sleepycode): uncomment when sleepycode is published to scoop
        ...
        // TODO(sleepycode): uncomment when sleepycode has github releases
        // const response = yield* httpOk.execute(
      ...
      const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        if (!result || result.code !== 0) {
          // TODO(sleepycode): restore choco-specific error when choco channel is supported
