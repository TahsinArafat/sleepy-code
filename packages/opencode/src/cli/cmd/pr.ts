...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run sleepycode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for sleepycode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found sleepycode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["sleepy", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting sleepycode...")
        UI.println()

        const sleepyArgs = sessionId ? ["-s", sessionId] : []
        const sleepyProcess = Process.spawn(["sleepy", ...sleepyArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await sleepyProcess.exited
        if (code !== 0) throw new Error(`sleepy exited with code ${code}`)
      },
    })
  },
})
