...
const issue = cmd({
  ...
  handler: (args) =>
    inInstance(async () => {
      ...
      // Resolved rather than guessed: a sleepycode process serving THIS directory advertises
      ...
      if (args.json) {
        ...
        process.stdout.write(
          JSON.stringify({
            ...
            // recover from `expired_api_key` without knowing whether sleepycode came
      ...
      // The endpoint belongs to whichever sleepycode process serves this project, and each
      ...
      if (!address) {
        UI.println("Start a session in this directory (or `sleepy serve --port <n>`) and issue again.")
      }
    }),
})
