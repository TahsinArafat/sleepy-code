...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall sleepycode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall SleepyCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @sleepy-ai/cli",
      pnpm: "pnpm uninstall -g @sleepy-ai/cli",
      bun: "bun remove -g @sleepy-ai/cli",
      // TODO(sleepycode): uncomment when published to these channels
      // brew: "brew uninstall sleepycode",
      // choco: "choco uninstall sleepycode",
      // scoop: "scoop uninstall sleepycode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using SleepyCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# sleepycode") || content.includes(".sleepycode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".sleepycode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".sleepycode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".sleepycode"))
    ) {
      continue
    }
...
async function cleanWindowsPath() {
  const installDir = path.join(os.homedir(), ".sleepycode", "bin")
  const script = `
    $installDir = $env:SLEEPYCODE_UNINSTALL_DIR
  ...
  const result = await Process.run(["powershell", "-ep", "Bypass", "-c", script], {
    nothrow: true,
    env: { SLEEPYCODE_UNINSTALL_DIR: installDir },
  })
