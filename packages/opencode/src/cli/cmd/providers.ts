...
import type { Hooks } from "@sleepy-ai/plugin"
...
async function sleepyLogin() {
  ...
  const method = sleepyHook.auth.methods[0]
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (remaining > 0) {
      prompts.log.error(t("cli.providers.sleepy_login.decrypt_retry", { remaining }))
    } else {
      prompts.log.error(t("cli.providers.sleepy_login.decrypt_exhausted"))
    }
  }
}
...
export const ProvidersLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "sleepycode auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in sleepycode.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via sleepycode.json options (profile, region, endpoint) or\n" +
...
export const ProvidersWhoamiCommand = cmd({
  ...
  async handler(_args) {
    ...
    if (!info) {
      prompts.log.error("Not logged in. Run `sleepy auth login` to log in.")
      return
    }
    if (info.type === "api" && info.metadata) {
      prompts.log.info(`Provider: Sleepy`)
