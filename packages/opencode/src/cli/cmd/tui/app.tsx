...
function rendererConfig(_config: TuiConfig.Info, plainTerminal: boolean): CliRendererConfig {
  const mouseEnabled = !plainTerminal && !Flag.SLEEPYCODE_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  useKeyboard((evt) => {
    if (!Flag.SLEEPYCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  onMount(() => {
    if (!Flag.SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR) return
  ...
  createEffect(() => {
    ...
    if (Flag.SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR && !orchestratorDirResolved()) return
  ...
  createEffect(() => {
    ...
    // common case: user runs `sleepy -s <id>` from a project dir) navigates the
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "workflow.list",
      category: "session",
      enabled: Flag.SLEEPYCODE_EXPERIMENTAL_WORKFLOW_TOOL,
      slash: {
        name: "workflows",
      },
      onSelect: () => {
  ...
  return (
    <box
      ...
      onMouseDown={(evt) => {
        ...
        if (
          Flag.SLEEPYCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT &&
    ...
    >
      <Show when={Flag.SLEEPYCODE_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={showHome() && !switchingOrchestrator()}>
        <Switch>
