...
import sleepycode from "./theme/sleepycode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@sleepy-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "sleepycode")
        draft.active = typeof active === "string" ? active : "sleepycode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.sleepycode, store.mode)
    })
...
const THEME_SCHEMA_URL = "https://mimo.xiaomi.com/sleepycode/theme.json"
