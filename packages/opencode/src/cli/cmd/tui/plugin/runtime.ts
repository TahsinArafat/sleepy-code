...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".sleepycode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".sleepycode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".sleepycode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.SLEEPYCODE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.SLEEPYCODE_PURE && config.plugin_origins?.length) {
        ...
        // File-based TUI plugins from .sleepycode/tui/*.{ts,tsx}
        const configDirs = [
          path.join(Global.Path.config, "tui"),
          path.join(cwd, ".sleepycode", "tui"),
        ]
