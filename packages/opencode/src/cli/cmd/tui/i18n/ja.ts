...
export const dict = {
  ...
  "tui.home.agreement.prefix": "SleepyCode をご利用いただくことで、",
  ...
  "tui.tips.free_api_sunset":
    "無料 API サービスは終了しました。{highlight}/login{/highlight} でログインしてください。Sleepy Token Plan を購読するか、サードパーティ API を設定して Sleepy Code をご利用ください。",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}sleepycode.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/sleepycode/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "SleepyCode は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.sleepycode/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.sleepycode/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "SleepyCode は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "SleepyCode はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.sleepycode/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.sleepycode/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて SleepyCode が機密ファイルを読まないようにできます",
  "tui.tips.run": "{highlight}sleepy run{/highlight} で非対話的なスクリプト実行ができます",
  "tui.tips.continue": "{highlight}sleepy --continue{/highlight} で前回のセッションを再開します",
  "tui.tips.attach_cli": "{highlight}sleepy run -f file.ts{/highlight} で CLI からファイルを添付できます",
  "tui.tips.format_json": "{highlight}--format json{/highlight} を使うとスクリプトで機械可読な出力を得られます",
  "tui.tips.serve": "{highlight}sleepy serve{/highlight} を実行するとヘッドレスで SleepyCode API にアクセスできます",
  "tui.tips.attach_server": "{highlight}sleepy run --attach{/highlight} で起動中のサーバに接続します",
  "tui.tips.upgrade": "{highlight}sleepy upgrade{/highlight} で最新バージョンに更新します",
  "tui.tips.auth_list": "{highlight}sleepy auth list{/highlight} で設定済みプロバイダを確認します",
  "tui.tips.agent_create": "{highlight}sleepy agent create{/highlight} でガイド付きエージェント作成を行います",
  "tui.tips.github_install": "{highlight}sleepy github install{/highlight} で GitHub workflow を設定します",
  ...
  "tui.tips.theme_files": "{highlight}.sleepycode/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.debug_config": "{highlight}sleepy debug config{/highlight} で設定の問題を調査します",
  ...
  "tui.tips.zen": "Sleepy Code と組み合わせて {highlight}/connect{/highlight} で厳選されたモデルを使えます",
  ...
  "tui.skill.sleepycode.description": "SleepyCode の機能・設定・コマンドに関するセルフドキュメント",
  ...
  "tui.dialog.free_api_sunset.message":
    "/login でログインしてください。Sleepy Token Plan を購読するか、サードパーティ API を設定して Sleepy Code をご利用ください。",
  ...
  "tui.model.sleepy_auto.name": "Sleepy Auto（Sleepy-V2.5 7月26日 18:00 まで無料 · UTC+8）",
  ...
  "tui.dialog.try_best.handoff.description": "未完了の作業をこのハーネスへ委任するよう Sleepy に依頼",
  ...
  "tui.toast.update_available.success": "SleepyCode v{{version}} に更新しました。アプリケーションを再起動してください。",
  ...
  "tui.voice.error.no_auth": "/connect で Sleepy アカウントに接続するか、voice.asr_model で別のプロバイダーを設定してください",
  ...
  "cli.providers.sleepy.recommended_hint": "推奨",
  "cli.providers.sleepy_free.hint": "匿名無料チャネル / sleepy-auto",
  ...
  "cli.providers.sleepy_login.decrypt_exhausted": "復号に失敗しました、最大再試行回数に達しました",
  ...
  "trust.capabilities": "Sleepy Code はここでファイルの読み取り、編集、実行が可能になります。",
  ...
  "skip_permissions.body":
    "--dangerously-skip-permissions を指定して起動しました。Sleepy Code は承認を求めずにファイルの読み取り・編集・実行やシェルコマンドの実行を行います。設定で明示的に拒否（deny）したルールのみが引き続き適用されます。発生したいかなる結果についてもあなた自身が全責任を負います。",
