...
import type { GlobalEvent } from "@sleepy-ai/sdk/v2"
import type { EventSource } from "./context/sdk"
import { win32DisableProcessedInput, win32InstallCtrlCGuard } from "./win32"
import { writeHeapSnapshot } from "v8"
import { TuiConfig } from "./config/tui"
import { SLEEPYCODE_PROCESS_ROLE, SLEEPYCODE_RUN_ID, ensureRunID, sanitizedProcessEnv } from "@/util/sleepy-process"
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start sleepycode tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start sleepycode in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      if (args["dangerously-skip-permissions"]) {
        ...
        process.env.SLEEPYCODE_DANGEROUSLY_SKIP_PERMISSIONS = "1"
      }

      const env = sanitizedProcessEnv({
        [SLEEPYCODE_PROCESS_ROLE]: "worker",
        [SLEEPYCODE_RUN_ID]: ensureRunID(),
      })
