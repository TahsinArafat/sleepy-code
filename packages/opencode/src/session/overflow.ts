...
// SLEEPYCODE_COMPACTION_TRIGGER_RATIO.
...
function budget(input: { cfg: Config.Info; model: Provider.Model }, hard: number, reserved: number) {
  const configured = input.cfg.compaction?.max_context ?? Flag.SLEEPYCODE_COMPACTION_MAX_CONTEXT
...
export function contextWindow(input: { cfg: Config.Info; model: Provider.Model }): Window {
  ...
  return {
    hard,
    effective,
    usable: Math.floor(effective * Flag.SLEEPYCODE_COMPACTION_TRIGGER_RATIO),
