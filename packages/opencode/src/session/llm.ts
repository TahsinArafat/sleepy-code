 ...
 *   (configured per-provider via `chunkTimeout` in sleepycode.json). This
 ...
 * format contract.
 *
 * This block is not appended when `SLEEPYCODE_DISABLE_CHECKPOINT` is on.
 *
...
function buildMemoryInstructions(projectID: ProjectID, memoryRoot: string): string {
  ...
  const checkpointEnabled = !Flag.SLEEPYCODE_DISABLE_CHECKPOINT
...
- A clearly durable cross-session fact emerges that you want available immediately${checkpointEnabled ? ", before the next checkpoint" : ""} → ## Discovered durable knowledge${checkpointEnabled
      ? `
    ...
    const buildSystemArray = Effect.fn("LLM.buildSystemArray")(function* (input: {
      ...
      system.push(
        [
          ...
          //
          // Below that, the fork deliberately differs from SystemPrompt.agent():
          // upstream is either/or (`agent.prompt ? agent.prompt : provider(model)`),
          // while the fork PREPENDS the model-family provider prompt to a primary
          // agent's own prompt, so custom primary agents (apex/phantom/pivot/forge/
          // stack, orchestrator) inherit the provider's base instructions. Hidden and
          // sub-agents keep agent-prompt-only, exactly as upstream does.
          // `input.system` and `input.user.system` are no longer composed here:
          // upstream moved them to the skill/instruction tail (system.push(...input.system))
          // and to turnContextMessages()/providerSystem respectively.
          ...(replaceAgent && input.user.system
            ? [input.user.system]
            : input.agent.prompt && (input.agent.hidden || input.agent.mode === "subagent")
              ? [input.agent.prompt]
              : [
                ...SystemPrompt.provider(input.model, input.user.harness),
      ...
      if (servesCheckpoint && !Flag.SLEEPYCODE_DISABLE_CHECKPOINT) {
        const projectID =
          (yield* Effect.try({
