...
You can extend your own capabilities by writing files to `.sleepycode/`. Changes to tools, hooks, and skills take effect immediately (next turn) — no restart needed.
...
```ts
// .sleepycode/tools/bash.ts — overrides built-in bash
import { tool } from "@sleepy-ai/plugin"
