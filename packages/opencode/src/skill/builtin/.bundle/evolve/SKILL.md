...
Before creating: check whether the extension already exists (`ls .sleepycode/tools .sleepycode/hooks .sleepycode/skills`). Prefer improving an existing one over adding a near-duplicate.
...
Need to redo a multi-agent run  → workflow (.sleepycode/workflows/*.js)
Need to change the UI           → TUI plugin (.sleepycode/tui/*.tsx)
```
...
Write to `.sleepycode/tools/<name>.ts`:
...
Write to `.sleepycode/hooks/<name>.ts` — export a Hooks object:
...
Write to `.sleepycode/skills/<name>/SKILL.md`:
...
|------|------|-----------|
| Tools | `.sleepycode/tools/*.ts` | next turn |
| Hooks | `.sleepycode/hooks/*.ts` | next turn |
| Skills | `.sleepycode/skills/*/SKILL.md` | next turn |
| Workflows | `.sleepycode/workflows/*.js` | on invoke |
| TUI | `.sleepycode/tui/*.tsx` | restart |
