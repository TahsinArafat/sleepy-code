...
- `~/.config/sleepycode/tui/*.{ts,tsx,js}` — global
- `<project>/.sleepycode/tui/*.{ts,tsx,js}` — project-local

Plugins can also be declared in `tui.json` (global `~/.config/sleepycode/tui.json`
or project `.sleepycode/tui.json`) via the `plugin` array (file URLs or npm specs).
...
```ts
// The full @sleepy-ai/sdk client — same as what plugins get on server side
