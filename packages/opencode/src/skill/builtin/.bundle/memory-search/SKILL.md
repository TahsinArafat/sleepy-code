...
Direct SQL access to sleepycode's trajectory database for structured analysis that the `memory` (BM25 over curated markdown) and `history` (FTS over raw messages) tools cannot perform — aggregation, filtering by tool/status/time, cross-session pattern detection, and execution chain inspection.
...
```bash
# Typically at this path. SLEEPYCODE_DB env var overrides if set.
sqlite3 -readonly ~/.local/share/sleepycode/sleepycode.db ".tables"
```
