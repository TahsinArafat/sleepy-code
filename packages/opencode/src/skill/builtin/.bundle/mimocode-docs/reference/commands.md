# SleepyCode Commands Reference

## CLI (`sleepy <command>`)

Invoked from the shell. `sleepy` with no command opens the TUI.

| Command | Purpose |
|---------|---------|
| `sleepy` | Launch the interactive TUI |
| `sleepy run` | Headless, non-interactive run (scripting/eval) |
| `sleepy mcp` | Manage / inspect MCP servers |
| `sleepy agent` | Manage agents |
| `sleepy models` | List available models |
| `sleepy providers` | List / manage providers |
| `sleepy account` (console) | Account / login console |
| `sleepy upgrade` | Update to the latest version |
| `sleepy uninstall` | Uninstall SleepyCode |
| `sleepy serve` | Run the server |
| `sleepy llm-server issue`/`list`/`revoke` | Mint and manage tokens that let a task reach this instance's models over `/v1`; it starts nothing — see @capability-api.md |
| `sleepy stats` | Usage statistics |
| `sleepy export` / `sleepy import` | Export / import sessions |
| `sleepy session` | Manage sessions |
| `sleepy github` / `sleepy pr` | GitHub / pull-request integration |
| `sleepy generate` | Code generation entry |
| `sleepy plugin` (plug) | Manage plugins |
| `sleepy db` | Database utilities |
| `sleepy acp` / `sleepy attach` | ACP / attach to a running session |
| `sleepy debug` | Debug utilities |
| `sleepy completion` | Generate shell completion script |

Run `sleepy <command> --help` for flags on any command.
...
For terminal compatibility, TUI rendering or lag, and local rendering over SSH with `sleepy serve` + `sleepy attach`, see @guide.md.

## Slash commands (inside the TUI)

Type `/` to see the commands available in the current context. You can also ask in chat, for example, “Which slash commands can I use?” or “How do I switch models?” SleepyCode will explain the relevant command without requiring you to remember its name.
...
| `/login` | — | Sign in to Xiaomi Sleepy |
| `/connect` | — | Connect or sign in to a model provider |
| `/logout` | — | Sign out of Xiaomi Sleepy |
...
| `/voice` | Toggle streaming voice input (requires `sox` and a Sleepy login) |
...
- Voice ASR (`sleepy-v2.5-asr`) is Sleepy-platform only; voice control (`sleepy-v2.5`) also runs on OpenRouter and compatible relays via the `voice` config (see config.md and the README voice section).
