...
SleepyCode implements the MCP client `sampling` capability
([spec](https://modelcontextprotocol.io/specification/2025-11-25/client/sampling)).
An MCP server can ask SleepyCode to run a model call on its behalf via
...
The Sleepy Cut MCP server extracts a video's audio track to a 16 kHz mono WAV and
needs a transcript. Instead of shipping its own provider credentials, it sends
the WAV to SleepyCode as MCP `AudioContent` and asks for a completion:
...
```json
{
  ...
  "params": {
    ...
    "maxTokens": 2048,
    "modelPreferences": { "hints": [{ "name": "sleepy-v2.5" }] }
  }
}
```

SleepyCode picks a configured, audio-capable model (preferring the hinted
`sleepy-v2.5`), asks the user to approve, runs the call, and returns the model's
text unchanged. `SLEEPY_API_KEY` is never read or needed by the server.

## Capability negotiation

SleepyCode declares `capabilities.sampling = {}` during `initialize`.
...
SleepyCode does exactly that. `includeContext` defaults to `none`: SleepyCode never
...
When nothing is eligible, SleepyCode returns a **structured error** naming every
...
SleepyCode is. It fails with `sampling stalled: the model produced no output` and
...
number.** Set `chunkTimeout` for a provider in `sleepycode.json` and it governs
...
Reaching this bound aborts the provider call as well as SleepyCode's wait
...
While the model call is in flight SleepyCode emits periodic
...
SDK's 60 s default gives up at 60 s while SleepyCode is still working, since
SleepyCode imposes no deadline of its own on the model call.
...
- **It reports observed output, but it is still not a fraction.** The model call
  ...
  actually produced — which is what lets a server tell "SleepyCode is alive" from
...
- **It never carries the model's text.** Partial content is deliberately withheld:
  ...
  channel, and treating `message` as one would be SleepyCode deciding that on the
...
- **It is necessary, not sufficient.** Resetting the timer is the *requester's*
  ...
  its own deadline. SleepyCode cannot make that decision for it.
- **It cannot extend SleepyCode's own bounds.** The notifications reset the peer's
...
SleepyCode reads it back off the request's `_meta`. No token means no notifications
at all.
...
A sampling request normally arrives **while SleepyCode is still waiting for that
...
2. SleepyCode runs the work on a fresh root Effect fiber, which shares no fiber,
...
>
> SleepyCode does not work around this, because it cannot: the id at risk belongs to
...
> it issues on a connection, SleepyCode does not learn of it, so that request keeps a
> model call running — and a paid one — until one of SleepyCode's own bounds aborts
...
- The model's text is returned **verbatim**. SleepyCode does not summarise or
  rewrite it.
...
that is now the only deadline SleepyCode imposes; the `"approval"`, `"model"` and
