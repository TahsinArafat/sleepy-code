...
Every SleepyCode server already serves an OpenAI-compatible surface over the models this
project has configured, mounted at `/v1`. There is no separate LLM server to start and no
daemon to manage: if a SleepyCode process is listening, the endpoints are there.
...
SleepyCode **defines** this interface (llm / asr / tts / voice design / voice clone); consumers
adapt to it.
...
behaves as it always did, credential-free on loopback — it is what `sleepy attach` over SSH
uses:

```bash
sleepy --port 4096                 # TUI plus a listener on a port you chose
sleepy serve --port 4096           # headless, same endpoints
```
...
`sleepy llm-server issue` can print a base URL. A spawned child should be **told** its
endpoint by whoever spawned it rather than picking one out of that directory.

## Minting a token

`sleepy llm-server` only mints and manages credentials; it starts nothing.

```bash
sleepy llm-server issue --capability transcription --json --label my-skill
sleepy llm-server issue --model anthropic/claude-haiku-4-5 --ttl 12h
sleepy llm-server list [--json]
sleepy llm-server revoke <id> | --all
```
...
- `--json` prints `api_key`, `id`, `base_url`, `expires_at`, `models`, and — with
  ...
  this installation, so a consumer can recover from expiry without knowing how SleepyCode was
...
The only preset voice currently available on the Sleepy TTS API is `"Chloe"`; other names
...
the `sleepy-v2.5-tts-voicedesign` and `sleepy-v2.5-tts-voiceclone` models.
...
| 502 | — | The upstream provider failed, not SleepyCode |
