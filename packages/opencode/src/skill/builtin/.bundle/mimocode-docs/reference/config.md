# SleepyCode Configuration Reference

## File locations & precedence

Config is JSON or JSONC. SleepyCode discovers it by walking up from the cwd to the worktree root, then falls back to global.

- Project: `.sleepycode/sleepycode.json` or `.sleepycode/sleepycode.jsonc`
- Global: `~/.config/sleepycode/sleepycode.jsonc` or `sleepycode.json` (XDG config dir). New installs seed `sleepycode.jsonc`.
- Extra config dirs are also searched via `$SLEEPYCODE_CONFIG_DIR`.

Project config merges **over** global. Always include `"$schema": "https://mimo.xiaomi.com/sleepycode/config.json"` for validation.

## On-disk data layout

Base directories resolve from `SLEEPY_HOME` if set (must be absolute → `<home>/{data,cache,config,state}`), otherwise from XDG:

| Kind | Default location | Holds |
|------|------------------|-------|
| data | `~/.local/share/sleepycode/` | memory, logs, `builtin_skills/<version>/`, bin |
| config | `~/.config/sleepycode/` | global `sleepycode.jsonc` / `sleepycode.json` |
| cache | `~/.cache/sleepycode/` | caches, downloaded bins |
| state | `~/.local/state/sleepycode/` | runtime state, including TUI recent/favorite models in `model.json` |

Memory files live under `~/.local/share/sleepycode/memory/`:
...
- `SLEEPY_HOME` — override all base dirs (absolute path).
- `SLEEPYCODE_CONFIG_DIR` — extra config directory to search.
- `SLEEPYCODE_PURE` — run without external plugins (same as `sleepy --pure`). Does **not** change models or Claude Code inheritance.
- `SLEEPYCODE_SLEEPY_ONLY` — pure-Sleepy mode: don't inherit Claude Code settings (CLAUDE.md, `~/.claude/skills`), don't read provider API keys from env, fall back to the sleepy-auto model.
- `SLEEPYCODE_DISABLE_LOG_ROTATION` — keep a single growing log file instead of rotating.
- `SLEEPYCODE_TEXT_TOOL_CALL_RETRY_LIMIT` — retries when a model emits a tool call as prose markup instead of a structured call (default 2).
- `SLEEPYCODE_EXPERIMENTAL_CRON` — scheduled prompts (cron/loop); **on by default**. `SLEEPYCODE_DISABLE_CRON` kills it at runtime. Tune loop keepalive with `SLEEPYCODE_LOOP_KEEPALIVE_BUDGET` (default 1) and `SLEEPYCODE_LOOP_KEEPALIVE_DELAY_S` (default 1200).
- `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_HEURISTIC` — shape-based compaction of bash output to save tokens; off by default.
- `SLEEPYCODE_DISABLE_BUILTIN_SKILLS`, `_COMPOSE_SKILLS`, `_EXTERNAL_SKILLS`, `_CLAUDE_CODE_SKILLS`, `_CODEX_SKILLS`, `_OPENCODE_SKILLS`, `_PROJECT_CONFIG`, `_CLAUDE_IMPORT` — feature toggles.
...
- A ref without `/` is a group name. If configured, SleepyCode is **provider-aware**: it prefers a member on the caller's current provider, otherwise falls back to the group's `default`.
...
Prefer a markdown file (`.sleepycode/agent/<name>.md`, body = system prompt) for defining a custom agent/mode — see the "Custom agents & modes" section in @guide.md. Use the `agent` config key for short, inline per-agent overrides.
...
| `server` | Config for `sleepy serve` |
