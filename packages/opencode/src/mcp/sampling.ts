 ...
 * `provider.ts:wrapSSE`, configurable per provider in `sleepycode.json`, default
 ...
 * records a real observation — "sleepy-v2.5-pro on Sleepy Router whose cold-path TTFT
  ...
  const call = Effect.tryPromise({
    try: (fiberSignal: AbortSignal) => {
      ...
      const stream = streamText({
        ...
        headers: { ...model.headers, "User-Agent": `sleepycode/${InstallationVersion}` },
