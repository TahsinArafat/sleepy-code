 ...
 * shapes SleepyCode already speaks.
 *
...
const ContentPart = z.discriminatedUnion("type", [
  ...
  z.object({
    // OpenAI's audio-input part, which `gpt-4o-audio-preview`, Sleepy, and Gemini all
...
export function transcriptionUnsupported(req: TranscriptionRequest): string | undefined {
  ...
  // a prompt. Measured against Sleepy: "ASR request must not include text parts".
 ...
 * Sleepy answers `mime type must be one of: audio/wav, audio/mpeg, audio/mp3. Got:
