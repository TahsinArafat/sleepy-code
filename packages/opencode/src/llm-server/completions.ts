 ...
 * (Sleepy, `gpt-4o-audio-preview`) have no such factory and are served by
 ...
 * on `sleepy-v2.5`, whose verbatim output was actually CLEANER than the dedicated
 * `sleepy-v2.5-asr` under `language: "en"`, which leaks `think>\n<chinese> `.
 *
...
export async function start(input: {
  ...
  // all. `src/plugin/sleepy.ts` supplies its provider's headers from `chat.headers`, and a
 ...
 * dedicated ASR endpoint REFUSES text parts (Sleepy answers 400 "ASR request must not
...
export async function transcribe(input: {
  ...
  if (Provider.modelKind(resolved.model) === "language") {
    ...
    if (typeof text !== "string" || text.length === 0) {
      // Measured on `sleepy-v2.5`: a reasoning model sometimes emits the whole transcript
