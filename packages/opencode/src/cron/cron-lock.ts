...
import { ensureSleepycodeGitignore } from "@/config/gitignore"
...
export const getLockFilePath = (dir?: string) => join(dir ?? process.cwd(), ".sleepycode", ".cron-lock")
...
export const tryAcquireSchedulerLock = (opts?: { dir?: string; lockIdentity?: string }) =>
  Effect.gen(function* () {
    ...
    yield* Effect.tryPromise({
      try: () => ensureSleepycodeGitignore(join(path, "..")),
      catch: () => undefined,
    }).pipe(Effect.orElseSucceed(() => undefined))
