...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".sleepycode-project-id")
  const idFile = nodePath.join(mainGit, "sleepycode-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .sleepycode-project-id
  ...
  if (!existing.includes(".sleepycode-project-id")) {
    await nodeFs.appendFile(excludeFile, "\n.sleepycode-project-id\n")
  }
}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.SLEEPYCODE_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        if (Flag.SLEEPYCODE_DISABLE_GIT) {
          return {
            id: ProjectID.global,
            worktree: directory,
            sandbox: directory,
        ...
        // turn, which has shipped as a bug for users opening sleepycode in $HOME.
      ...
      if (Flag.SLEEPYCODE_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
