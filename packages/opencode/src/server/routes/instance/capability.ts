 ...
 * `chat.headers` hook (`src/plugin/sleepy.ts`), applied on the agent's LLM path. A separate
...
export const CapabilityRoutes = lazy(() =>
  new Hono()
     ...
     *
     * `AuthMiddleware` only enforces Basic auth when `SLEEPYCODE_SERVER_PASSWORD` is set, and
    ...
    .onError((err, c) => {
      ...
      // server's problem, but they still need to tell "SleepyCode broke" from "the provider
