...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-sleepycode-directory")
  out.delete("x-sleepycode-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-sleepycode-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-sleepycode-proxy-url", socket(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }
