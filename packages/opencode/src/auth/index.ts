...
export const OAUTH_DUMMY_KEY = "sleepycode-oauth-dummy-key"
 ...
 *
 * Kept in memory on purpose: the previous channel was `SLEEPYCODE_AUTH_CONTENT`, and anything in
 * the environment is readable by every child the engine spawns — the bash tool alone turns
 * `echo $SLEEPYCODE_AUTH_CONTENT` into a credential dump, and a sibling can still read
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const all = Effect.fn("Auth.all")(function* () {
      const raw = injected ?? process.env.SLEEPYCODE_AUTH_CONTENT
      if (raw) {
        try {
