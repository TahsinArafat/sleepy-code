...
//
// Env:
//   SLEEPY_FDS_AK / SLEEPY_FDS_SK            credentials (required to upload)
//   SLEEPY_FDS_ENDPOINT                    upload+signing host (default cnbj1 prod)
//   SLEEPY_FDS_BUCKET / SLEEPY_FDS_PREFIX    object layout (default sleepycode/sleepycode)
//
...
export const FDS_ENDPOINT = process.env.SLEEPY_FDS_ENDPOINT || "cnbj1-fds.api.xiaomi.net"
export const FDS_BUCKET = process.env.SLEEPY_FDS_BUCKET || "sleepycode"
export const FDS_PREFIX = process.env.SLEEPY_FDS_PREFIX || "sleepycode"
...
// Full object name under the bucket, e.g. "sleepycode/releases/latest".
...
export async function uploadFile(localFile: string, subPath: string, contentType?: string) {
  const accessKey = process.env.SLEEPY_FDS_AK
  const secret = process.env.SLEEPY_FDS_SK
  if (!accessKey || !secret) throw new Error("SLEEPY_FDS_AK / SLEEPY_FDS_SK must be set to upload to FDS")
