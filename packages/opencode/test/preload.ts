...
const dir = path.join(os.tmpdir(), "sleepycode-test-data-" + process.pid)
...
const fixtureRoot = path.join(process.cwd(), ".sleepycode-test-fixtures-" + process.pid)
await fs.mkdir(fixtureRoot, { recursive: true })
process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"] = fixtureRoot
afterAll(async () => {
...
process.env["SLEEPYCODE_MODELS_PATH"] = path.join(import.meta.dir, "tool", "fixtures", "models-api.json")
...
process.env["SLEEPYCODE_TEST_MANAGED_CONFIG_DIR"] = testManagedConfigDir
process.env["SLEEPYCODE_DISABLE_DEFAULT_PLUGINS"] = "true"

// Write the cache version file to prevent global/index.ts from clearing the cache
const cacheDir = path.join(dir, "cache", "sleepycode")
...
delete process.env["SLEEPYCODE_SERVER_PASSWORD"]
delete process.env["SLEEPYCODE_SERVER_USERNAME"]
delete process.env["SLEEPY_HOME"]

// Use in-memory sqlite
process.env["SLEEPYCODE_DB"] = ":memory:"
...
// SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR; the orchestrator test suites exercise the
// feature, so enable it here (Flag is read once at import — must be set first).
process.env["SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR"] = "true"
