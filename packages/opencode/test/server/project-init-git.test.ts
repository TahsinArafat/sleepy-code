...
// inherit a parent .git. We temporarily set Flag.SLEEPYCODE_SERVER_PASSWORD to
// bypass the middleware cwd containment check and include auth headers.
const TEST_PASSWORD = "init-git-test"
const authHeader = `Basic ${Buffer.from(`sleepycode:${TEST_PASSWORD}`).toString("base64")}`

describe("project.initGit endpoint", () => {
  test("initializes git and reloads immediately", async () => {
    const prevFlag = (Flag as any).SLEEPYCODE_SERVER_PASSWORD
    ;(Flag as any).SLEEPYCODE_SERVER_PASSWORD = TEST_PASSWORD
    try {
      ...
      try {
        const init = await app.request("/project/git/init", {
          method: "POST",
          headers: {
            "x-sleepycode-directory": tmp.path,
        ...
        expect(await Filesystem.exists(path.join(tmp.path, ".git", "sleepycode"))).toBe(false)
      ...
      ;(Flag as any).SLEEPYCODE_SERVER_PASSWORD = prevFlag
    }
  })
