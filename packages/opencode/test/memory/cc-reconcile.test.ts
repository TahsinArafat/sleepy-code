  ...
  itLive.live("indexes CC files under scope='cc' with frontmatter type", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const sleepyRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(sleepyRoot, { recursive: true, force: true }))
...
Body about cache.`
        ...
        yield* Effect.promise(() => reconcileMemory({ sleepy: sleepyRoot, cc: ccBase }))
  ...
  itLive.live("union prune: deleting one CC file leaves sleepy rows untouched", () =>
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const memory = yield* Memory.Service
        const sleepyRoot = yield* memory.root()
        yield* Effect.promise(() => fs.rm(sleepyRoot, { recursive: true, force: true }))
        yield* Effect.promise(() => fs.mkdir(path.join(sleepyRoot, "global"), { recursive: true }))
        yield* Effect.promise(() => fs.writeFile(path.join(sleepyRoot, "global", "m.md"), "sleepy body"))
...
cc body`,
        ...
        expect(rows.length).toBe(1)
        expect(rows[0].scope).toBe("global") // sleepy row preserved
      }),
    ),
  )
...
body`,
        ...
        yield* Effect.promise(() => reconcileMemory({ sleepy: sleepyRoot }))
        expect(
          Database.use((db) =>
            db.select().from(MemoryFtsTable).where(eq(MemoryFtsTable.scope, "cc")).all(),
          ).length,
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.promise(() =>
          reconcileMemory({ sleepy: sleepyRoot, cc: "/definitely/not/a/real/path/abc123" }),
        )
