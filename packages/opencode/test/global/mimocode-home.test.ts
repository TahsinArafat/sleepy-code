...
describe("SLEEPY_HOME end-to-end", () => {
  test("sets all four paths under the profile root", async () => {
    await using tmp = await tmpdir()
    const result = await runWorker({ SLEEPY_HOME: tmp.path })
  ...
  test("auto-creates all four subdirs under a nonexistent root", async () => {
    ...
    const result = await runWorker({ SLEEPY_HOME: nested })
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  test("two profiles have fully isolated data (write in A, not visible in B, A survives restart)", async () => {
    ...
    const resultA = await runWorker({ SLEEPY_HOME: a.path })
    ...
    const resultB = await runWorker({ SLEEPY_HOME: b.path })
    ...
    const resultA2 = await runWorker({ SLEEPY_HOME: a.path })
  ...
  test("relative path causes startup failure with clear error", async () => {
    const result = await runWorker({ SLEEPY_HOME: "./relative-path" })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.stderr).toMatch(/SLEEPY_HOME must be an absolute path/)
  })
