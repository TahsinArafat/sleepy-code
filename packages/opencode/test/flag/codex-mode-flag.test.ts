...
function read(value?: string) {
  ...
  if (value === undefined) delete env.SLEEPYCODE_CODEX_MODE
  else env.SLEEPYCODE_CODEX_MODE = value
  const result = Bun.spawnSync({
    cmd: [
      process.execPath,
      "-e",
      'import { Flag } from "./src/flag/flag.ts"; process.stdout.write(String(Flag.SLEEPYCODE_CODEX_MODE))',
    ],
    cwd: process.cwd(),
    env,
  })
...
describe("SLEEPYCODE_CODEX_MODE", () => {
