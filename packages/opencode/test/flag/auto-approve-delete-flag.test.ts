...
const original = process.env.SLEEPYCODE_AUTO_APPROVE_DELETE

function set(value?: string) {
  if (value === undefined) delete process.env.SLEEPYCODE_AUTO_APPROVE_DELETE
  else process.env.SLEEPYCODE_AUTO_APPROVE_DELETE = value
}

afterEach(() => set(original))

describe("SLEEPYCODE_AUTO_APPROVE_DELETE", () => {
  test("keeps the forced-ask confirmation by default", () => {
    set(undefined)
    expect(Flag.SLEEPYCODE_AUTO_APPROVE_DELETE).toBe(false)
  })
  ...
  test("tracks env writes made after the module was first imported", () => {
    set("true")
    expect(Flag.SLEEPYCODE_AUTO_APPROVE_DELETE).toBe(true)
    set("false")
    expect(Flag.SLEEPYCODE_AUTO_APPROVE_DELETE).toBe(false)
    set("true")
    expect(Flag.SLEEPYCODE_AUTO_APPROVE_DELETE).toBe(true)
