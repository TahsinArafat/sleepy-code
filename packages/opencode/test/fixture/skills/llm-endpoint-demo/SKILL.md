...
SleepyCode's provider config or credential store.
...
1. **Make sure a server is running for this project.** Ask it:
   ...
   ```
   <sleepycode> llm-server status --json
   ```

   `<sleepycode>` is however THIS installation is invoked. Do not assume `sleepy` is
   on `PATH` — it frequently is not, for example under `npx` or a source checkout.
   If nothing is running, start one in the background:

   ```
   <sleepycode> llm-server &
   ```
...
2. **Mint a token scoped to this task.** Ask by CAPABILITY, not by model name:
   ...
   ```
   <sleepycode> llm-server issue --capability speech --label llm-endpoint-demo --json
   <sleepycode> llm-server issue --capability transcription --json
   <sleepycode> llm-server issue --capability chat --json
   ```

   The response includes the `model` it resolved, so set your own env var from that
   rather than hard-coding an id — a skill that names `sleepy-v2.5-tts` only runs where
