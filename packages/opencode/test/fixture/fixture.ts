...
export function withTmpdirOutsideGit<T>(fn: () => Promise<T>): Promise<T> {
  const prev = process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"]
  process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"] = outsideGitTmpRoot()
  return fn().finally(() => {
    if (prev !== undefined) process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"] = prev
    else delete process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"]
  })
}
...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const prevRoot = options?.outsideGit ? process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"] : undefined
  if (options?.outsideGit) process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"] = outsideGitTmpRoot()
  const dirpath = sanitizePath(
    path.join(process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"] ?? os.tmpdir(), "sleepycode-test-" + Math.random().toString(36).slice(2)),
  )
  ...
  if (options?.git) {
    ...
    await $`git config user.email "test@sleepycode.test"`.cwd(dirpath).quiet()
    await $`git config user.name "Test"`.cwd(dirpath).quiet()
    await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "sleepycode.json"),
      JSON.stringify({
    ...
    [Symbol.asyncDispose]: async () => {
      try {
        ...
        if (options?.outsideGit) {
          if (prevRoot !== undefined) process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"] = prevRoot
          else delete process.env["SLEEPYCODE_TEST_TMPDIR_ROOT"]
        }
      }
    },
...
export function tmpdirScoped(options?: { git?: boolean; config?: Partial<Config.Info>; outsideGit?: boolean }) {
  return Effect.gen(function* () {
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@sleepycode.test")
