...
const managedConfigDir = process.env.SLEEPYCODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "sleepycode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "sleepycode.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "sleepycode.jsonc"),
        `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "sleepycode.json"))
...
test("migrates old opencode.ai $schema URL to mimo.xiaomi.com", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await load()
      const content = await Filesystem.readText(path.join(tmp.path, "sleepycode.json"))
      expect(content).toContain('"$schema": "https://mimo.xiaomi.com/sleepycode/config.json"')
...
test("preserves JSONC comments when injecting $schema", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await load()
      const content = await Filesystem.readText(path.join(tmp.path, "sleepycode.jsonc"))
      expect(content).toContain("$schema")
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["SLEEPYCODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:SLEEPYCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["SLEEPYCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["SLEEPYCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "sleepycode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .sleepycode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".sleepycode")
...
test("loads agents from .sleepycode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".sleepycode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .sleepycode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".sleepycode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .sleepycode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".sleepycode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("sleepycode command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".sleepycode", "command", "dup.md"),
        `---
description: sleepycode version
---
from sleepycode`,
      )
    },
...
test("does not try to install dependencies in read-only SLEEPYCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.SLEEPYCODE_CONFIG_DIR
  process.env.SLEEPYCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.SLEEPYCODE_CONFIG_DIR
    else process.env.SLEEPYCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable SLEEPYCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .sleepycode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".sleepycode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .sleepycode config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "sleepycode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .sleepycode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "sleepycode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".sleepycode")
      ...
      await Filesystem.write(
        path.join(local, "sleepycode.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
// Note: preload.ts sets SLEEPYCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .sleepycode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .sleepycode directory config enables it
      const opencodeDir = path.join(dir, ".sleepycode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "sleepycode.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "sleepycode.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.sleepycode/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.sleepycode/plugin/demo.ts", "file:///project/.sleepycode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.sleepycode/plugin/demo.ts"])
  })
...
describe("SLEEPYCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"]
    process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .sleepycode/ directories when flag is set", async () => {
    const originalEnv = process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"]
    process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .sleepycode directory with a command
          const opencodeDir = path.join(dir, ".sleepycode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .sleepycode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SLEEPYCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["SLEEPYCODE_CONFIG_DIR"]
      process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["SLEEPYCODE_CONFIG_DIR"]
      } else {
        process.env["SLEEPYCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("SLEEPYCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SLEEPYCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["SLEEPYCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["SLEEPYCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from SLEEPYCODE_CONFIG_DIR, not project
...
describe("SLEEPYCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in SLEEPYCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SLEEPYCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["SLEEPYCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["SLEEPYCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["SLEEPYCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in SLEEPYCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SLEEPYCODE_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.sleepycode.managed.test",
          PayloadType: "ai.sleepycode.managed",
