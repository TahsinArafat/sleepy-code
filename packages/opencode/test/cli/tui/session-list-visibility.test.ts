...
const scaffold = Effect.gen(function* () {
  ...
  const plain = yield* sessions.create({
    parentID: root.id as SessionID,
    title: "build: 在 sleepycode 引擎侧实现「memory 写入开关」",
  })
  ...
  const writerHost = yield* sessions.create({
    parentID: root.id as SessionID,
    title: "checkpoint-writer: Previous checkpoint: /Users/mi/.local/share/sleepycode/memory/sessions/ses_x/checkpoint.md",
  })
