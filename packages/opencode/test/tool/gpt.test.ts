...
const codexMode = process.env.SLEEPYCODE_CODEX_MODE

beforeEach(() => {
  delete process.env.SLEEPYCODE_CODEX_MODE
})

afterEach(() => {
  if (codexMode === undefined) delete process.env.SLEEPYCODE_CODEX_MODE
  else process.env.SLEEPYCODE_CODEX_MODE = codexMode
})
...
describe("isMcpToolSearchEnabled", () => {
  test("defaults to GPT models and allows explicit non-GPT opt-in", () => {
    ...
    expect(isMcpToolSearchEnabled(false, undefined, "sleepy-v2.5")).toBe(false)
    expect(isMcpToolSearchEnabled(false, undefined, "sleepy-v2.5-pro")).toBe(false)
    expect(isMcpToolSearchEnabled(false, undefined, "sleepy-v2.5-pro-ultraspeed")).toBe(false)
    expect(isMcpToolSearchEnabled(false, undefined, "mimo-v2-pro")).toBe(false)
    expect(isMcpToolSearchEnabled(false, undefined, "sleepy-v2.6")).toBe(false)
    expect(isMcpToolSearchEnabled(false, undefined, "sleepy-ptc-test")).toBe(false)
    expect(isMcpToolSearchEnabled(false, undefined, "sleepy-v2.6-ptc")).toBe(false)
  ...
  test("enables every non-GPT model when process Codex mode is enabled", () => {
    process.env.SLEEPYCODE_CODEX_MODE = "true"
    expect(isMcpToolSearchEnabled(false, undefined, "claude-opus-4-6")).toBe(true)
    expect(isMcpToolSearchEnabled(false, undefined, "sleepy-v2.6")).toBe(true)
    expect(isMcpToolSearchEnabled(false, undefined, "sleepy-v2.6-ptc")).toBe(true)
  })

  test("does not force MCP tool search for GPT models when process Codex mode is disabled", () => {
    process.env.SLEEPYCODE_CODEX_MODE = "false"
  ...
  test("allows the resolved session mode to override the process mode", () => {
    ...
    expect(isMcpToolSearchEnabled(false, "codex", "sleepy-v2.6")).toBe(true)
    expect(isMcpToolSearchEnabled(false, "codex", "sleepy-v2.6-ptc")).toBe(true)
    ...
    expect(isMcpToolSearchEnabled(false, "auto", "sleepy-v2.6")).toBe(true)
    expect(isMcpToolSearchEnabled(false, "auto", "sleepy-v2.6-ptc")).toBe(true)
    expect(isMcpToolSearchEnabled(false, "default", "claude-opus-4-6")).toBe(false)
    expect(isMcpToolSearchEnabled(false, "default", "sleepy-v2.6")).toBe(false)
    expect(isMcpToolSearchEnabled(false, "default", "gpt-5.2")).toBe(true)
    expect(isMcpToolSearchEnabled(true, "default", "sleepy-v2.6")).toBe(true)
  })
})

describe("usesGPTToolset", () => {
  test("uses the normal toolset for Sleepy models regardless of API transport", () => {
    expect(usesGPTToolset("sleepy-v2.5")).toBe(false)
    expect(usesGPTToolset("sleepy-v2.5-pro")).toBe(false)
    expect(usesGPTToolset("sleepy-v2.5-pro-ultraspeed")).toBe(false)
    expect(usesGPTToolset("mimo-v2-pro")).toBe(false)
    expect(usesGPTToolset("sleepy-v2.6")).toBe(false)
    expect(usesGPTToolset("sleepy-ptc-test")).toBe(false)
    expect(usesGPTToolset("sleepy-v2.6-ptc")).toBe(false)
  })
  ...
  test("uses the GPT toolset for every non-GPT model in process Codex mode", () => {
    ...
    expect(usesGPTToolset("sleepy-v2.6")).toBe(true)
    expect(usesGPTToolset("sleepy-v2.6-ptc")).toBe(true)
  })
  ...
  test("allows the resolved session mode to override the process mode", () => {
    ...
    expect(usesGPTToolset("sleepy-v2.6", "codex")).toBe(true)
    expect(usesGPTToolset("sleepy-v2.6-ptc", "codex")).toBe(true)
    expect(usesGPTToolset("deployment-primary", "codex", "sleepy-v2.6", "sleepy")).toBe(true)
    expect(usesGPTToolset("deployment-primary", "codex", "sleepy-v2.6-ptc", "sleepy")).toBe(true)
    expect(usesGPTToolset("gpt-5.2", "auto")).toBe(true)
    expect(usesGPTToolset("sleepy-v2.6-ptc", "auto")).toBe(false)
    expect(usesGPTToolset("claude-opus-4-6", "auto")).toBe(false)
    process.env.SLEEPYCODE_CODEX_MODE = "true"
    expect(usesGPTToolset("claude-opus-4-6", "auto")).toBe(true)
    expect(usesGPTToolset("sleepy-v2.6", "auto")).toBe(true)
    expect(usesGPTToolset("sleepy-v2.6-ptc", "auto")).toBe(true)
    expect(usesGPTToolset("claude-opus-4-6", "default")).toBe(false)
    expect(usesGPTToolset("sleepy-v2.6", "default")).toBe(false)
