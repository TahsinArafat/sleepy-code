...
describe("createPipeline", () => {
  ...
  test("SLEEPYCODE_BASH_RAW=1 bypasses the pipeline", () => {
    const prev = process.env.SLEEPYCODE_BASH_RAW
    process.env.SLEEPYCODE_BASH_RAW = "1"
    try {
      const drop: CleanPlugin = { name: "drop", apply: () => "" }
      const result = createPipeline([drop]).run("payload", { command: "ls" })
      expect(result.text).toBe("payload")
    } finally {
      if (prev === undefined) delete process.env.SLEEPYCODE_BASH_RAW
      else process.env.SLEEPYCODE_BASH_RAW = prev
    }
  })
