...
// prefers `spawn`. This drives the real headless CLI against the live sleepy router
// and reads the emitted operation out of the structured tool part.
//
// Measured 2026-07-28, sleepy/sleepy-v2.5, 6 trials per prompt per arm, actor denied by
...
// the live router + a real key in ~/.config/sleepycode/sleepycode.json). Run with:
...
// A scratch SLEEPY_HOME: the real sleepy provider (test/preload.ts strips provider
...
function scratchHome() {
  ...
  const user = JSON.parse(readFileSync(path.join(os.homedir(), ".config", "sleepycode", "sleepycode.json"), "utf8"))
  if (!user.provider?.sleepy?.options?.apiKey) throw new Error("no sleepy provider/key in ~/.config/sleepycode/sleepycode.json")
  writeFileSync(
    path.join(home, "config", "config.json"),
    JSON.stringify({
      model: "sleepy/sleepy-v2.5",
      permission: { actor: { "**": "deny" } },
      provider: { sleepy: user.provider.sleepy },
    }),
  )
  return home
}

// The emitted operation, taken from the tool part's structured input only. sleepy
...
async function trial(home: string, prompt: string) {
  const proc = Bun.spawn(
    ["bun", "run", "--conditions=browser", "./src/index.ts", "run", "--model", "sleepy/sleepy-v2.5", "--format", "json", prompt],
    { cwd: PKG, env: { ...process.env, SLEEPY_HOME: home }, stdout: "pipe", stderr: "ignore" },
  )
  const ops: string[] = []
