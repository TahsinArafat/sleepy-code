...
describe("tool.registry", () => {
  it.live("loads tools from .sleepycode/tool (singular)", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const opencode = path.join(dir, ".sleepycode")
  ...
  it.live("loads tools from .sleepycode/tools (plural)", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const opencode = path.join(dir, ".sleepycode")
  ...
  it.live("keeps the reserved MCP search tool when a custom tool conflicts", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const tools = path.join(dir, ".sleepycode", "tools")
        yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
        yield* Effect.promise(() =>
          Bun.write(
