...
async function isolateZshDotfiles() {
  ...
  const zdotdir = path.join(os.tmpdir(), `sleepycode-zdotdir-${Math.random().toString(36).slice(2)}`)
    ...
    try {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // The tmpdir git fixture sets user.name=Test / user.email=test@sleepycode.test.
          expect(result.metadata.output).toContain("GIT_AUTHOR_NAME=Test")
          expect(result.metadata.output).toContain("GIT_AUTHOR_EMAIL=test@sleepycode.test")
          expect(result.metadata.output).toContain("GIT_COMMITTER_NAME=Test")
          expect(result.metadata.output).toContain("GIT_COMMITTER_EMAIL=test@sleepycode.test")
        },
      })
    } finally {
      restoreEnv(saved)
    ...
    try {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // user.name=Test / user.email=test@sleepycode.test.
          expect(result.metadata.output).toContain("GIT_AUTHOR_EMAIL=test@sleepycode.test")
          expect(result.metadata.output).toContain("GIT_COMMITTER_NAME=Test")
          expect(result.metadata.output).toContain("GIT_COMMITTER_EMAIL=test@sleepycode.test")
        },
...
describe("tool.bash permissions", () => {
  ...
  each("skips bash_delete for a delete confined to the OS temp dir", async () => {
    await using tmp = await tmpdir()
    const scratch = await fs.mkdtemp(path.join(os.tmpdir(), "sleepycode-bash-tmpdel-"))
  ...
  each("still asks for bash_delete for a project file even when the project is under temp", async () => {
    ...
    const projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), "sleepycode-bash-tmpproj-"))
  ...
  if (process.platform === "win32") {
    ...
    for (const item of ps) {
      test(
        ...
        withShell(item, async () => {
          const key = "SLEEPYCODE_TEST_MISSING"
