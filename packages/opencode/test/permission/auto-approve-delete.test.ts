...
const originalDangerouslySkipPermissions = Flag.SLEEPYCODE_DANGEROUSLY_SKIP_PERMISSIONS

beforeEach(() => {
  Flag.SLEEPYCODE_DANGEROUSLY_SKIP_PERMISSIONS = false
})

afterEach(async () => {
  Flag.SLEEPYCODE_DANGEROUSLY_SKIP_PERMISSIONS = originalDangerouslySkipPermissions
...
describe.serial("Permission auto-approve-delete runtime toggle", () => {
  ...
  it.live(
    ...
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        Flag.SLEEPYCODE_DANGEROUSLY_SKIP_PERMISSIONS = true
        const perm = yield* Permission.Service
