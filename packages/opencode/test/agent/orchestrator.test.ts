...
// force-sets SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR=true for the suite). `Flag` reads
...
function listAgentNames(flag: boolean): string[] {
  ...
  try {
    const env: Record<string, string> = {
      ...
      SLEEPYCODE_DB: ":memory:",
      SLEEPYCODE_DISABLE_DEFAULT_PLUGINS: "true",
      SLEEPYCODE_TEST_TMPDIR_ROOT: path.join(root, "tmp"),
    }
    delete env.SLEEPYCODE_EXPERIMENTAL
    delete env.SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR
    if (flag) env.SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR = "true"
