...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when @sleepy-ai/cli is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "@sleepy-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when @sleepy-ai/cli is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "@sleepy-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when @sleepy-ai/cli is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "@sleepy-ai/cli@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has @sleepy-ai/cli", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
  ...
  describe("latest", () => {
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          expect(req.url).toContain(encodeURIComponent("@sleepy-ai/cli"))
    ...
    test("resolves version from FDS for curl method", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "curl" && args.includes("https://sleepycode.cnbj1.mi-fds.com/sleepycode/sleepycode/releases/latest"))
            return "v0.1.1\n"
          return ""
        },
      )
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package", async () => {
      ...
      expect(capturedArgs).toContain("@sleepy-ai/cli@2.0.0")
    })
