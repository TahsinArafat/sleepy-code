...
describe("postinstall", () => {
  ...
  test("creates .sleepycode binary cache from platform package", async () => {
    ...
    const cached = path.join(binDir, ".sleepycode")
  ...
  test("prints migration notice to stdout", async () => {
    ...
    expect(stdout).toContain("Recommended: install SleepyCode natively for a better")
  ...
  test("exits with error when binary package is missing", async () => {
    ...
    expect(result.stdout.toString()).toContain("Recommended: install SleepyCode natively")
  })
