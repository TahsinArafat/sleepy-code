...
const root = path.resolve(import.meta.dir, "../../src/skill/builtin/.bundle/sleepycode-docs")

describe("sleepycode-docs provider guidance", () => {
  ...
  test("documents the implemented OpenAI-compatible adapter and safe merge rules", async () => {
    ...
    expect(providers).toContain("sleepy models PROVIDER_ID")
  })
  ...
  test("documents the effective global config precedence", async () => {
    ...
    expect(providers).toContain("`config.json`, `sleepycode.json`, then `sleepycode.jsonc`; later files win")
    expect(providers).toContain("create `sleepycode.jsonc` when none exists")
  })
  ...
  test("keeps a newly configured API model in TUI recents without clobbering state", async () => {
    ...
    expect(providers).toContain("$SLEEPY_HOME/state/model.json")
    ...
    expect(providers).toContain("Write the recent state only after `sleepy models PROVIDER_ID`")
...
describe("sleepycode-docs TUI troubleshooting", () => {
  test("routes rendering issues to actionable terminal and SSH guidance", async () => {
    ...
    expect(guide).toContain("sleepy serve --port 4096")
    expect(guide).toContain("ssh -N -L 4096:127.0.0.1:4096 user@remote-host")
    expect(guide).toContain("sleepy attach http://127.0.0.1:4096")
 ...
 */
describe("sleepycode-docs permission arity", () => {
  test("classifies mcp_sampling as glob-keyed, not action-only", async () => {
    ...
    expect(permissions).toContain('"mcp_sampling": { "*": "ask", "sleepy-cut": "allow" }')
  })
