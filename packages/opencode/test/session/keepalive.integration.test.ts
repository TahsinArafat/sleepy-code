...
const originalCronFlag = Flag.SLEEPYCODE_EXPERIMENTAL_CRON
afterEach(async () => {
  ;(Flag as { SLEEPYCODE_EXPERIMENTAL_CRON: boolean }).SLEEPYCODE_EXPERIMENTAL_CRON = originalCronFlag
...
beforeEach(() => {
  ...
  delete process.env.SLEEPYCODE_LOOP_KEEPALIVE_BUDGET
  delete process.env.SLEEPYCODE_LOOP_KEEPALIVE_DELAY_S
  ;(Flag as { SLEEPYCODE_EXPERIMENTAL_CRON: boolean }).SLEEPYCODE_EXPERIMENTAL_CRON = true
})
...
describe("cron-bridge keepalive sweep", () => {
  ...
  it.live("budget=0 ends loop immediately on first turn without a re-arm", () =>
    Effect.gen(function* () {
      process.env.SLEEPYCODE_LOOP_KEEPALIVE_BUDGET = "0"
