...
// because the registry entry is gated behind SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR,
...
describe("orchestrator fleet roster — e2e on the wire", () => {
  test("live peer children appear in the roster the orchestrator model receives", async () => {
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "sleepycode.json"), tmpConfig(`${server.url.origin}/v1`))
      },
    })
