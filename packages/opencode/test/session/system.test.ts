...
const dynamicSystemPrompt = process.env.SLEEPYCODE_ENABLE_DYNAMIC_SYSTEM_PROMPT
const codexMode = process.env.SLEEPYCODE_CODEX_MODE

beforeEach(() => {
  process.env.SLEEPYCODE_ENABLE_DYNAMIC_SYSTEM_PROMPT = "true"
  delete process.env.SLEEPYCODE_CODEX_MODE
})

afterEach(() => {
  if (dynamicSystemPrompt === undefined) delete process.env.SLEEPYCODE_ENABLE_DYNAMIC_SYSTEM_PROMPT
  else process.env.SLEEPYCODE_ENABLE_DYNAMIC_SYSTEM_PROMPT = dynamicSystemPrompt
...
describe("session.system", () => {
  test("does not render dynamic environment information by default", async () => {
    delete process.env.SLEEPYCODE_ENABLE_DYNAMIC_SYSTEM_PROMPT
  ...
  test("the explicit harness selects the prompt for Sleepy regardless of API transport", () => {
    ...
    const sleepy = ProviderTest.model({ id: ModelID.make("sleepy-v2.6"), api: { id: "sleepy-v2.6" } as never })
    expect(SystemPrompt.provider(sleepy, "codex")[0]).toContain("You are Codex")
    expect(SystemPrompt.provider(sleepy, "codex")[0]).toContain("tools.apply_patch")
    expect(SystemPrompt.provider(sleepy, "default")[0]).not.toContain("You are Codex")
    expect(SystemPrompt.provider(sleepy, "default")[0]).not.toContain("tools.apply_patch")

    const responses = ProviderTest.model({
      id: ModelID.make("sleepy-v2.6-ptc"),
      api: { id: "sleepy-v2.6-ptc" } as never,
    })
  ...
  test("uses the GPT prompt for GPT models and the normal prompt for Sleepy models", () => {
    ...
    const prompts = ["sleepy-v2.5", "sleepy-v2.5-pro", "sleepy-v2.5-pro-ultraspeed", "mimo-v2-pro", "sleepy-v2.6"].map(
      (id) =>
    ...
    expect(SystemPrompt.provider(ProviderTest.model({ id: ModelID.make("sleepy-v2.6-ptc") }))[0]).toBe(normal)
  })
  ...
  test("Codex mode forces the GPT prompt for every model", () => {
    ...
    expect(SystemPrompt.provider(ProviderTest.model({ id: ModelID.make("sleepy-v2.6") }))[0]).toBe(gpt)
    expect(SystemPrompt.provider(ProviderTest.model({ id: ModelID.make("sleepy-v2.6-ptc") }))[0]).toBe(gpt)
  })
    ...
    await using tmp = await tmpdir({
      ...
      init: async (dir) => {
        for (const [name, description] of [
          ...
          const skillDir = path.join(dir, ".sleepycode", "skill", name)
          await Bun.write(
