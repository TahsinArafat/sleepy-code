...
describe("session.llm system prompt — memory-instructions guard", () => {
  test("main agent (no agentID) — '# Memory system' IS appended", async () => {
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "sleepycode.json"), tmpConfig(providerID, `${server.url.origin}/v1`))
      },
    })
  ...
  test("SLEEPYCODE_DISABLE_CHECKPOINT=true — memory instructions are not appended", async () => {
