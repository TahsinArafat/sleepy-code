...
function sleepycodeConfig(baseURL: string, extra?: Record<string, unknown>) {
...
describe("Manual /rebuild: on-the-spot rebuild driven through SessionPrompt.command", () => {
    ...
    async () => {
      ...
      try {
        await using tmp = await tmpdir({
          git: true,
          init: (dir) => Bun.write(path.join(dir, "sleepycode.json"), sleepycodeConfig(llm.origin)),
        })
  ...
  test(
    "SLEEPYCODE_DISABLE_CHECKPOINT=true → compacts immediately without starting or waiting for a writer",
    async () => {
      const previous = process.env.SLEEPYCODE_DISABLE_CHECKPOINT
      process.env.SLEEPYCODE_DISABLE_CHECKPOINT = "true"
      const llm = startLLM("should-not-be-used-as-a-reply")
      ...
      try {
                  ...
                  const notice = after
                    ...
                    .find(
                      (p) =>
                        ...
                        p.text.includes("SLEEPYCODE_DISABLE_CHECKPOINT"),
                    )
        ...
        if (previous === undefined) delete process.env.SLEEPYCODE_DISABLE_CHECKPOINT
        else process.env.SLEEPYCODE_DISABLE_CHECKPOINT = previous
        GlobalBus.off("event", onEvent)
        await llm.stop()
      }
    },
        ...
        await using tmp = await tmpdir({
          ...
          init: (dir) =>
            Bun.write(
              ...
              sleepycodeConfig(llm.origin, { memory: { disable_write: true } }),
            ),
        })
