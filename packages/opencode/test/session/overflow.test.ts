...
describe("isOverflow", () => {
  ...
  // Flag.SLEEPYCODE_COMPACTION_TRIGGER_RATIO), so it reserves headroom uniformly
...
describe("SLEEPYCODE_COMPACTION_TRIGGER_RATIO", () => {
  afterEach(() => {
    delete process.env["SLEEPYCODE_COMPACTION_TRIGGER_RATIO"]
  })

  test("a decimal moves the trigger", () => {
    process.env["SLEEPYCODE_COMPACTION_TRIGGER_RATIO"] = "0.75"
    expect(usable({ cfg: mockCfg(), model: createModel({ context: 200_000 }) })).toBe(150_000)
  })

  test("a percentage is equivalent to the decimal", () => {
    process.env["SLEEPYCODE_COMPACTION_TRIGGER_RATIO"] = "75%"
    expect(usable({ cfg: mockCfg(), model: createModel({ context: 200_000 }) })).toBe(150_000)
  })

  test("1 lets usage fill the whole working window", () => {
    process.env["SLEEPYCODE_COMPACTION_TRIGGER_RATIO"] = "1"
  ...
  test("applies on top of the max_context budget rather than replacing it", () => {
    process.env["SLEEPYCODE_COMPACTION_TRIGGER_RATIO"] = "0.5"
    const model = createModel({ context: 200_000 })
  ...
  test.each(["0", "-0.5", "1.5", "150%", "abc", ""])("ignores %p and keeps the 0.9 default", (value) => {
    process.env["SLEEPYCODE_COMPACTION_TRIGGER_RATIO"] = value
