 ...
 *
 * A real `sleepy serve` process, a real HTTP socket, and the demo skill's real scripts as
...
async function serve(directory: string) {
  ...
  const proc = Bun.spawn(
    ...
    ],
    {
      ...
      stdout: "pipe",
      stderr: "pipe",
      env: { ...process.env, SLEEPYCODE_DISABLE_PROVIDER_ENV: "1" },
    },
  )
...
async function withEndpoint<T>(transcript: string, fn: (ctx: Ctx) => Promise<T>): Promise<T> {
  ...
  try {
    writeFileSync(path.join(dir, "sleepycode.json"), JSON.stringify(projectConfig(up.port)))
