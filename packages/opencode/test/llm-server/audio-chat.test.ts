...
/**
 * Audio carried over `POST /v1/chat/completions`, which is how Sleepy, Gemini's
...
describe("synthesis over chat completions", () => {
  test("returns the vendor's audio, and puts the text in an assistant message", async () => {
    // The placement is the convention's requirement, not a preference: Sleepy does not
  ...
  test("an upstream refusal surfaces its reason, including the param half", async () => {
    // Sleepy answers `message: "Param Incorrect"` with the actual cause in `param`.
...
describe("kind derivation routes each model to one endpoint", () => {
  ...
  test.skip("superseded: a multimodal chat model is now served as a transcription fallback", async () => {
    // The guard that matters: `sleepy-v2.5` and every Gemini declare audio INPUT
...
describe("multimodal fallback for transcription", () => {
   ...
   * preferred but not required. Measured on `sleepy-v2.5`: its verbatim output was
   * actually cleaner than `sleepy-v2.5-asr` under `language: "en"`.
   *
  ...
  test("suppresses thinking so the transcript lands in content, not reasoning", async () => {
    ...
    // `content: null` some of the time. `thinking: {type: "disabled"}` is Sleepy's control,
  ...
  test("a dedicated ASR model gets no instruction and no thinking control", async () => {
    // The opposite shape, on the same route. Sleepy answers 400 "ASR request must not
  ...
  test("reports a legible failure when the model answers as reasoning instead of content", async () => {
    // Measured on `sleepy-v2.5`: the transcript sometimes arrives as `reasoning_content`
...
describe("plugin hooks on the model path", () => {
   ...
   *
   * Some providers are authenticated by a plugin: `src/plugin/sleepy.ts` supplies its headers
  ...
  async function withPlugin<T>(
    ...
    try {
      ...
      await Bun.write(
        path.join(tmp.path, "sleepycode.json"),
        JSON.stringify({
...
describe("upload media types", () => {
  test("normalises the aliases platforms actually report", () => {
    // `File` reports a wav as `audio/x-wav` on some platforms, and Sleepy rejects that
