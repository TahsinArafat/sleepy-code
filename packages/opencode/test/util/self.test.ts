...
describe("self invocation", () => {
  ...
  test("carries the entry script when the runtime is the executable", () => {
    const previous = process.env["SLEEPYCODE_BIN_PATH"]
    delete process.env["SLEEPYCODE_BIN_PATH"]
    try {
      ...
      if (previous != null) process.env["SLEEPYCODE_BIN_PATH"] = previous
    }
  })

  test("honours SLEEPYCODE_BIN_PATH, the override bin/sleepy already reads", () => {
    const previous = process.env["SLEEPYCODE_BIN_PATH"]
    process.env["SLEEPYCODE_BIN_PATH"] = "/opt/pinned/sleepy"
    try {
      // A pinned binary replaces the whole invocation: no runtime, no script.
      expect(Self.argv("llm-server", "status")).toEqual(["/opt/pinned/sleepy", "llm-server", "status"])
    } finally {
      if (previous === undefined) delete process.env["SLEEPYCODE_BIN_PATH"]
      else process.env["SLEEPYCODE_BIN_PATH"] = previous
    }
  })

  test("never emits the bare string `sleepy`, which may not be a command", () => {
    // The whole point: an npx or node_modules launch has no `sleepy` on PATH.
    const line = Self.commandLine("llm-server", "issue")
    expect(line.startsWith("sleepy ")).toBe(false)
  ...
  test("quotes a path containing spaces so the line stays runnable", () => {
    ...
    const previous = process.env["SLEEPYCODE_BIN_PATH"]
    process.env["SLEEPYCODE_BIN_PATH"] = "/Applications/My App/bin/sleepy"
    try {
      expect(Self.commandLine("llm-server")).toBe("'/Applications/My App/bin/sleepy' llm-server")
    } finally {
      if (previous === undefined) delete process.env["SLEEPYCODE_BIN_PATH"]
      else process.env["SLEEPYCODE_BIN_PATH"] = previous
    }
  ...
  test("leaves an ordinary path unquoted", () => {
    const previous = process.env["SLEEPYCODE_BIN_PATH"]
    process.env["SLEEPYCODE_BIN_PATH"] = "/usr/local/bin/sleepy"
    try {
      expect(Self.commandLine("llm-server", "status")).toBe("/usr/local/bin/sleepy llm-server status")
    } finally {
      if (previous === undefined) delete process.env["SLEEPYCODE_BIN_PATH"]
      else process.env["SLEEPYCODE_BIN_PATH"] = previous
    }
  ...
  test("escapes an embedded single quote rather than producing a broken line", () => {
    const previous = process.env["SLEEPYCODE_BIN_PATH"]
    process.env["SLEEPYCODE_BIN_PATH"] = "/home/o'brien/bin/sleepy"
    try {
      const line = Self.commandLine()
      expect(line).toBe(`'/home/o'\\''brien/bin/sleepy'`)
    } finally {
      if (previous === undefined) delete process.env["SLEEPYCODE_BIN_PATH"]
      else process.env["SLEEPYCODE_BIN_PATH"] = previous
    }
