...
test("credential scrub applies only to inherited baseline", () => {
  const env = makeChildProcessEnv(() => ({
    SLEEPYCODE_AUTH_CONTENT: "inherited-secret",
    SLEEPYCODE_CONFIG_CONTENT: "inherited-config",
    USER_VAR: "kept",
  }))

  expect(env.resolve({ SLEEPYCODE_AUTH_CONTENT: "explicit-secret" })).toEqual({
    USER_VAR: "kept",
    SLEEPYCODE_AUTH_CONTENT: "explicit-secret",
  })
})
...
test("all inherited external process paths use childProcessEnv", async () => {
  ...
  expect(uninstall).toContain("env: { SLEEPYCODE_UNINSTALL_DIR: installDir }")
