...
SleepyCode supports connecting to any mainstream LLM provider API.
...
SleepyCode does not support the built-in macOS Terminal (Terminal.app). If the interface is misaligned, flickers, or has other rendering issues, use [iTerm2](https://iterm2.com/) or the VS Code integrated terminal instead:
...
If the TUI lags when run directly over SSH, render it locally and run only the SleepyCode server on the remote host. Start the server from the remote project directory:

```bash
# Remote host
sleepy serve --port 4096
...
sleepy attach http://127.0.0.1:4096
```
...
`sleepy models <provider>` prints, per model, the window SleepyCode resolved and the token count
...
Compose is SleepyCode's structured workflow for specs-driven development, orchestrating the full lifecycle from spec to shipped code.
...
SleepyCode ships with four built-in workflows:
...
**Custom workflows:** Place a `.js` file in `.sleepycode/workflows/` or `.claude/workflows/` to define your own, or override a built-in by using the same name (e.g. `.sleepycode/workflows/compose.js`).

### Builtin Skills

Skills are reusable instruction sets that teach agents how to handle specific tasks (e.g. generating PDFs, writing academic papers, searching arXiv). For a new task, SleepyCode searches available non-Compose skills by exact name, localized alias, and BM25 relevance. High-confidence matches are loaded automatically; uncertain matches are ranked for the agent to assess. In the TUI, type `/` to browse the autocomplete list or invoke a skill directly with `/<skill-name>` — mentioning two or more skills in a single message auto-loads them and injects a multi-skill orchestration plan.

SleepyCode bundles the following builtin skills:
...
| `sleepycode-docs` | Self-documenting reference for SleepyCode features, commands, providers, and configuration |
...
**Overriding a builtin skill:** Create a skill with the same `name` in your project (`.sleepycode/skills/<name>/SKILL.md`) or personal skill directory (`~/.claude/skills/`, `~/.opencode/skills/`, etc.). User skills discovered later in the scan order override builtins with the same name.
...
| Variable | Effect |
|----------|--------|
| `SLEEPYCODE_DISABLE_BUILTIN_SKILLS=true` | Disable all builtin skills |
| `SLEEPYCODE_DISABLE_OFFICIAL_SKILLS=true` | Disable only the office/media skills: `docx-official`, `pdf-official`, `pptx-official`, `xlsx-official`, `html-to-video-pipeline` |
| `SLEEPYCODE_DISABLE_SLASH_SKILLS=true` | Hide skills from TUI `/` autocomplete without disabling them |

The first two options remove the corresponding skills from the agent's available skill list entirely — they will not appear in context and cannot be invoked. `SLEEPYCODE_DISABLE_SLASH_SKILLS` affects only TUI autocomplete; the skills remain available to agents.
...
SleepyCode uses JSON/JSONC config files with published JSON Schemas for autocompletion and validation.

### File Locations

| File | Project-level | Global |
|------|--------------|--------|
| Main config | `.sleepycode/sleepycode.jsonc` (also `.json`) | `~/.config/sleepycode/sleepycode.jsonc` (also `.json`) |
| TUI config | `.sleepycode/tui.json` | `~/.config/sleepycode/tui.json` |
| Auth credentials | — | `~/.local/share/sleepycode/auth.json` |

> On Windows, XDG paths fall under `%LOCALAPPDATA%\sleepycode\`. You can override all paths with `SLEEPY_HOME`.

### JSON Schemas

SleepyCode auto-injects a `$schema` field when it first loads your config, so your editor gets completions and validation out of the box:

| Config | Schema URL |
|--------|-----------|
| `sleepycode.jsonc` / `sleepycode.json` | `https://mimo.xiaomi.com/sleepycode/config.json` |
| `tui.json` | `https://mimo.xiaomi.com/sleepycode/tui.json` |
...
Beyond config files, SleepyCode stores runtime data under XDG paths (or `$SLEEPY_HOME`):

| Directory | Default (Linux) | Contents |
|-----------|----------------|----------|
| data | `~/.local/share/sleepycode/` | SQLite database, auth credentials (`auth.json`), memory, logs |
| state | `~/.local/state/sleepycode/` | TUI preferences (`kv.json`), recent models (`model.json`) |
| cache | `~/.cache/sleepycode/` | Language servers, cached model catalog, skills |

To remove stored credentials, delete `auth.json` from the data directory. On macOS, XDG data defaults to `~/Library/Application Support/sleepycode/`.
...
```jsonc
{
  "$schema": "https://mimo.xiaomi.com/sleepycode/config.json",
...
- Preserve the base URL and model ID exactly as supplied. SleepyCode does not require a known provider and you should not add or remove `/v1` unless the endpoint requires it.
...
Put user-wide settings in `~/.config/sleepycode/sleepycode.jsonc` (or `sleepycode.json` in the same directory), or project-only settings in `.sleepycode/sleepycode.jsonc` (or `.json`), and merge them with any existing configuration. Because `apiKey` is stored as plaintext, keep the file readable only by your user and never commit it. Run `sleepy models` or use the TUI model picker to verify the configured model.
...
sleepy --dangerously-skip-permissions

# Headless
sleepy run --dangerously-skip-permissions "your prompt"

# Or via environment variable (any surface)
SLEEPYCODE_DANGEROUSLY_SKIP_PERMISSIONS=1 sleepy
```
