...
install_dir="${SLEEPYCODE_INSTALL_DIR:-$HOME/.sleepycode/bin}"
...
(
  ...
  SLEEPYCODE_CHANNEL=local SLEEPYCODE_VERSION=local bun run script/build.ts --single
)

binary="$package_dir/dist/sleepycode-$platform-$arch/bin/sleepy"
if [[ ! -f "$binary" ]]; then
...
mkdir -p "$install_dir"
temporary="$install_dir/.sleepy.$$"
trap 'rm -f "$temporary"' EXIT
install -m 755 "$binary" "$temporary"
mv -f "$temporary" "$install_dir/sleepy"
trap - EXIT

echo "Installed sleepy to $install_dir/sleepy"
"$install_dir/sleepy" --version
