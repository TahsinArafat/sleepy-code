<#
.SYNOPSIS
    SleepyCode installer for Windows.
.DESCRIPTION
    Downloads and installs SleepyCode to $env:USERPROFILE\.sleepycode\bin,
...
$Arch = if ($env:PROCESSOR_ARCHITECTURE -eq "ARM64") { "arm64" }
    ...
    else { Write-Err "SleepyCode requires a 64-bit operating system." }
...
$FdsBase = if ($env:SLEEPY_FDS_BASE) { $env:SLEEPY_FDS_BASE.TrimEnd('/') } else {
    "https://sleepycode.cnbj1.mi-fds.com/sleepycode/sleepycode"
}
...
# Warn about old versions that don't support sleepy upgrade via install.ps1
...
if ($MajMin -match '^\d+\.\d+\.\d+$') {
    ...
    if ($semver -le 104) {
        Write-Host ""
        Write-Host "WARNING: Installing v$Version via install.ps1 will cause 'sleepy upgrade' to not function properly." -ForegroundColor Yellow
...
$Staging = $false
if ($env:SLEEPYCODE_INSTALL_DIR) {
    $InstallDir = $env:SLEEPYCODE_INSTALL_DIR
    $Staging = $true
} else {
    $InstallDir = Join-Path $env:USERPROFILE ".sleepycode\bin"
}

if (-not $Staging) {
    $Existing = Get-Command sleepy -ErrorAction SilentlyContinue
    if ($Existing) {
        $InstalledVersion = & sleepy --version 2>$null
        if ($InstalledVersion -eq $Version) {
            Write-Host "SleepyCode v$Version is already installed." -ForegroundColor DarkGray
            Exit-Install 0
        }
...
$BinName = if (Test-Path (Join-Path $TmpDir "sleepy.exe")) { "sleepy.exe" } else { "sleepy" }
try {
    Move-Item -Path (Join-Path $TmpDir $BinName) -Destination (Join-Path $InstallDir "sleepy.exe") -Force -ErrorAction Stop
} catch {
    Remove-Item -Recurse -Force $TmpDir -ErrorAction SilentlyContinue
    Write-Err "Failed to install binary. If SleepyCode is currently running, please close it and retry.`n$($_.Exception.Message)"
}
Remove-Item -Recurse -Force $TmpDir -ErrorAction SilentlyContinue
