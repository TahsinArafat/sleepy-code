...
本文是 SleepyCode provider retry 的单一设计来源。实现位于 packages/opencode/src/session/retry.ts，配置 schema 位于 packages/opencode/src/config/config.ts 与 packages/opencode/src/config/provider.ts。
