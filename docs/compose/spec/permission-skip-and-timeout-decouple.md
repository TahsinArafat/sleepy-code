...
`SLEEPYCODE_SKIP_ALL_FORCED_ASK_TIMEOUT_MS` maps to the initial `permissionAskTimeoutMs` value when set to a positive integer. When unset or 0, `permissionAskTimeoutMs` initializes to `null`.
...
- [x] T2: Env var compat — `SLEEPYCODE_SKIP_ALL_FORCED_ASK_TIMEOUT_MS` maps to `permissionAskTimeoutMs` initial value. acceptance: new Instance's `permissionAskTimeout()` returns the env-derived value (covers: S2.3)
