...
1. CI shard 4 failed from cross-file env pollution: `skill.test.ts` sets `SLEEPYCODE_DISABLE_BUILTIN_SKILLS` at module top-level and never restores; the Flag getter reads env lazily. Fixed with the save/clear/restore pattern from #1850's `available-permission.test.ts`. Lesson: any test needing builtin-bundle extraction must defensively clear that flag.
