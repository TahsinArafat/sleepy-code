...
plan mode" moved to `sleepycode-docs`, which loads only when someone asks how
SleepyCode works.
...
from .sleepycode/tool (singular)` timing out at 5000ms under parallel load;
...
3. `sleepycode-docs` routing keys off the frontmatter `description` (BM25 over name
...
**2. It is already dead outside the TUI.** `sleepy run` denies both plan tools
...
i.e. the one Sleepy's own models get; `anthropic.txt` / `gpt.txt` / `codex.txt` /
...
`sleepycode-docs`, which is loaded exactly when a user asks how SleepyCode itself
works (`skill_search` BM25 over name + description, or explicit `/sleepycode-docs`),
...
- `sleepycode-docs/SKILL.md` frontmatter `description` — add mode / keybinding
  ...
  nothing to match.
- `sleepycode-docs/SKILL.md:18` — the Agents / modes row states that only the user
  enters a mode, that no tool switches into plan, and that `plan_exit` is the
  agent's one move from inside plan.
- `sleepycode-docs/reference/commands.md:130` — under Keybindings, the concrete
...
`sleepycode-docs/reference/guide.md:114` and `config.md:92` mention plan only as a
Compose-legacy skill name and an agent-config key; both stay accurate and are
left alone. The localized `tui.skill.sleepycode-docs.description` strings are the
...
- `packages/opencode/src/skill/builtin/.bundle/sleepycode-docs/SKILL.md` and
