...
reference document the field for skill authors; `sleepycode-docs` records that
...
from documentation skills such as `sleepycode-docs`, which also state that the
...
- `sleepycode-docs` records that `/compose-next` is user-invocable only and that
...
- [x] T7: Record in `sleepycode-docs` that `/compose-next` is user-invocable only
