...
**What was built** — Added a top-level optional boolean `auto_worktree` to `Config.Info`. The default is off: omitting the key or setting `false` means the Auto-Worktree Notice is never injected. Setting `true` preserves the previous soft-hint behavior (once per primary root session after a main-worktree mutation). Conflict detection and `POST /experimental/worktree/auto` remain outside this flag. Bundled `sleepycode-docs` config table and the generated JS SDK types list the new key.
