...
- **`reasoning part rs_… not found`** — a server-side Responses API rejection, seen even
  ...
  specifically against a **custom `baseURL` proxy** (Sleepy Router), not `api.openai.com`.
...
1. **Missing `include` in base options.** SleepyCode's base `options()` set `store:false` +
...
2. **Blanket item-id strip in the fetch wrapper (proxy-incompatible).** `provider.ts` had a
   ...
   before serialization. SleepyCode's fork was stuck on the old implementation.
...
- `provider/transform.ts` (+): extracted `mapProviderOptions()` helper (also reused by the existing
  ...
  - Deliberate divergence from upstream: dropped `@ai-sdk/amazon-bedrock/mantle` from the npm list
    (SleepyCode uses `@ai-sdk/amazon-bedrock`, and the old code never stripped bedrock). Kept
    `openai` + `azure` to match prior SleepyCode behavior.
...
`@ai-sdk/google@3.0.73` patch (pops empty Gemini `model` entries). SleepyCode's filter
