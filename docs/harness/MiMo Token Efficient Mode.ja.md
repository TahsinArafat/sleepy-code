...
- **単一フラグ、デフォルト無効** — `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` がクリーンアップパイプラインに入る唯一のスイッチで、デフォルト無効。そうでなければ Raw で直通する。
...
**単一フラグ、デフォルト無効** — `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` は独立したフラグで、デフォルトは無効。`SLEEPYCODE_EXPERIMENTAL=1` から派生しない。明示的オプトインで、デフォルト出力を静かに変更するのを避ける。
