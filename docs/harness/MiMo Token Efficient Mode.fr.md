...
- **Un seul flag, désactivé par défaut** — `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` est le seul interrupteur pour entrer dans le pipeline de nettoyage, désactivé par défaut ; sinon la sortie passe directement en Raw.
...
**Un seul flag, désactivé par défaut** — `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` est un flag autonome qui contrôle l'interrupteur, désactivé par défaut, non dérivé de `SLEEPYCODE_EXPERIMENTAL=1`. L'opt-in explicite évite de modifier silencieusement la sortie par défaut.
