...
- **Single flag, off by default** — `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` is the only switch that enters the cleanup pipeline, and it's disabled by default; otherwise output goes straight through Raw.
...
**Single flag, off by default** — `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` is a standalone flag controlling the switch, off by default, not derived from `SLEEPYCODE_EXPERIMENTAL=1`. Explicit opt-in avoids silently changing the default output.
