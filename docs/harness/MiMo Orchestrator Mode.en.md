...
**Off by default**: the whole capability is gated behind a single flag `SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR` (see §6). When off, SleepyCode behaves exactly as before — no Orchestrator mode, no `session` tool, no approval routing, no workspace switching.
...
Orchestrator session (globally unique, see §5)
   ...
   ▼
coordinate / integrate (git merge each child's sleepycode/* branch) / report to user
```
...
- A child is a peer, not an in-session subagent — you can **fully attach** into any child session to view/take over, just like `sleepy -c <id>`.
...
- `isolate` — when on, the child runs in **its own git worktree** of `dir`'s repo (branch `sleepycode/<task>`), so multiple children editing the same repo don't collide with each other or with the Orchestrator. Use it for "will edit files, possibly concurrently"; leave it off for read-only/single-writer work, or a non-git dir (which degrades to running directly in `dir`).

The worktree is created/removed in `dir`'s own repo Instance (cross-project correct); a child worktree lives at `<data>/worktree/<projID>/<task-slug>`, on branch `sleepycode/<task-slug>`.

### 3.2 Integration & cleanup

- An isolated child's commits live on its own `sleepycode/<...>` branch. The Orchestrator integrates them itself with git (it has `bash`): `git log <branch>` / `git diff <base>...<branch>` / `git merge-tree` to preview conflicts → `git merge <branch>` (or cherry-pick). Find a child's branch via `git worktree list` / `git branch --list 'sleepycode/*'`.
...
- No matter which directory you launch SleepyCode from, **switching into Orchestrator mode** switches the TUI's working directory to this global directory and lands on the **single** root Orchestrator session there (find-or-create).
...
- Default **OFF**; set `SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR=true` to enable (the umbrella `SLEEPYCODE_EXPERIMENTAL=1` enables it too).
...
1. Enable the feature: `SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR=true` (or `SLEEPYCODE_EXPERIMENTAL=1`).
2. Launch SleepyCode and press **Tab** to cycle to **Orchestrator** mode — the working directory switches automatically to the global Orchestrator workspace and lands on the single Orchestrator session.
...
6. When satisfied, have the Orchestrator merge/integrate each isolated child's `sleepycode/*` branch.
...
| flag definition | `packages/opencode/src/flag/flag.ts` (`SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR`) |
