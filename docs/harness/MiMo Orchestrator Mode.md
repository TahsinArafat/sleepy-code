...
**默认关闭**：整套能力由单一 flag `SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR` 门控（见第 6 章）。关闭时 SleepyCode 与从前完全一致——没有 Orchestrator 模式、没有 `session` 工具、没有审批路由、没有工作区切换。
...
Orchestrator 会话（全局唯一，见 §5）
   ...
   ▼
协调 / 集成（git merge 各 child 的 sleepycode/* 分支）/ 汇报给用户
```

- 每个 child 是一个**独立会话**（有自己的 session id、任务面板、记忆），以 `mode: "peer"` 在**后台**运行。
- Orchestrator 派发后**立即返回**，不轮询；child 完成时通过 inbox 通知**主动唤醒**它。
- child 是 peer，不是 in-session 的 subagent —— 用户可以像 `sleepy -c <id>` 一样**完整 attach** 进任意 child 会话查看/接管。
...
- `isolate` —— 打开后，child 在 `dir` 所属仓库里跑在**它自己的 git worktree**（分支 `sleepycode/<任务>`），这样多个 child 编辑同一仓库时互不冲突、也不与 Orchestrator 冲突。适合"会改文件、且可能并发"的场景；只读/单写、或非 git 目录则关闭（非 git 时自动降级为直接在 `dir` 里跑）。

worktree 在 `dir` 所属仓库自己的 Instance 上创建/删除（跨项目正确）；child worktree 位于 `<data>/worktree/<projID>/<task-slug>`，分支为 `sleepycode/<task-slug>`。

### 3.2 集成与清理

- 一个 isolated child 的提交在它自己的 `sleepycode/<...>` 分支上。Orchestrator 自己用 git 集成（它有 `bash`）：`git log <branch>` / `git diff <base>...<branch>` / `git merge-tree` 预览冲突 → `git merge <branch>`（或 cherry-pick）。用 `git worktree list` / `git branch --list 'sleepycode/*'` 找 child 分支。
...
- 无论从哪个目录启动 SleepyCode，**切到 Orchestrator 模式**都会把 TUI 的工作目录切到这个全局目录，并落到那里**唯一的**根 Orchestrator 会话（find-or-create）。
...
- 默认 **OFF**；设 `SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR=true` 开启（伞形 `SLEEPYCODE_EXPERIMENTAL=1` 也会一并开启）。
- **两个承重门控**让功能在关闭时彻底消失：
...
1. 开启功能：`SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR=true`（或 `SLEEPYCODE_EXPERIMENTAL=1`）。
2. 启动 SleepyCode，按 **Tab** 循环到 **Orchestrator** 模式——工作目录会自动切到全局 Orchestrator 工作区，落到唯一的 Orchestrator 会话。
...
6. 满意后让 Orchestrator 把各 isolated child 的 `sleepycode/*` 分支合并集成。
...
| flag 定义 | `packages/opencode/src/flag/flag.ts`（`SLEEPYCODE_EXPERIMENTAL_ORCHESTRATOR`） |
