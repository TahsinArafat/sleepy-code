...
- **单 flag、默认关** — `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` 是进入清理管线的唯一开关，且默认关闭，否则走 Raw 直出。
...
**单 flag、默认关** — `SLEEPYCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` 一个独立 flag 控制开关，默认关闭，不被 `SLEEPYCODE_EXPERIMENTAL=1` 派生。显式 opt\-in，避免静默改变默认输出。
