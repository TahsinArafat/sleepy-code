...
SleepyCode is an AI coding assistant that runs locally and can use powerful tools, including shell execution, file operations, and network access.

### No Sandbox

SleepyCode does **not** treat its permission system as a security sandbox. Permissions help users review and control actions, but they are not an isolation boundary.

If you need isolation, run SleepyCode in a disposable container, virtual machine, or similarly restricted environment. Review project instructions and third-party tool or MCP server configuration before allowing them to execute.

### Server Mode

Server mode is opt-in. By default it binds to a loopback address and may be accessed by other processes on the same machine. Set `SLEEPYCODE_SERVER_PASSWORD` to enable HTTP Basic Authentication. SleepyCode refuses to bind to a non-loopback address without a password unless the user explicitly passes `--no-auth`.
...
If you cannot use the GitHub form, contact the team privately at [support-sleepy@xiaomi.com](mailto:support-sleepy@xiaomi.com) with the subject `[SleepyCode Security] <brief summary>`. This address is a general team support mailbox rather than a dedicated security response service.
